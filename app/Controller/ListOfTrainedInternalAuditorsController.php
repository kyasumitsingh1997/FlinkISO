...
class ListOfTrainedInternalAuditorsController extends AppController {
    ...
    public function index() {
        ...
        $this->ListOfTrainedInternalAuditor->recursive = 1;
    ...
    public function add_ajax() {
        ...
        if ($this->request->is('post')) {
            if($this->request->data['ListOfTrainedInternalAuditor']['trainer_type'] == 0 && ($this->request->data['ListOfTrainedInternalAuditor']['employee_id'] == NULL || $this->request->data['ListOfTrainedInternalAuditor']['employee_id'] == -1 )){
                $this->Session->setFlash(__('Please select employee.'));
                $this->redirect(array('action' => 'lists'));
            }elseif($this->request->data['ListOfTrainedInternalAuditor']['trainer_type'] == 1 && $this->request->data['ListOfTrainedInternalAuditor']['name'] == ''){
                $this->Session->setFlash(__('Please add name'));
                $this->redirect(array('action' => 'lists'));
            }elseif($this->request->data['ListOfTrainedInternalAuditor']['employee_id'] != -1 && $this->request->data['ListOfTrainedInternalAuditor']['employee_id'] != NULL){
                $employees = $this->_get_employee_list();
                $this->request->data['ListOfTrainedInternalAuditor']['name'] = $employees[$this->request->data['ListOfTrainedInternalAuditor']['employee_id']];
            }
