			...
			<div class="row">
				<div class="col-md-4 pull-right">
					<div class="input-group pull-left">
						<?php echo $this->Form->create('MeetingTopic',array('role'=>'form','class'=>'no-padding no-margin','id'=>'search-form','action'=>'search')); ?>
...
<?php foreach ($meetingTopics as $meetingTopic): ?>
	<tr>
<td width="15"><?php echo $this->Form->checkbox('rec_ids_'.$i,array('label'=>false,'value'=>$meetingTopic['MeetingTopic']['id'],'multiple'=>'checkbox','class'=>'rec_ids')); ?></td>		<td class=" actions">
		<div class="btn-group">
		<button type="button" data-toggle="dropdown" class="dropdown-toggle btn  btn-sm btn-default "><i class="fa fa-wrench" aria-hidden="true"></i></button>
				...
				<li><?php echo $this->Html->link(__('View / Upload Evidance'), array('action' => 'view', $meetingTopic['MeetingTopic']['id'])); ?></li>
				<li><?php echo $this->Html->link(__('Edit'), array('action' => 'edit', $meetingTopic['MeetingTopic']['id'])); ?></li>
				<li><?php echo $this->Form->postLink(__('Delete'), array('action' => 'delete', $meetingTopic['MeetingTopic']['id']),array('class'=>''), __('Are you sure you want to delete this record ?', $meetingTopic['MeetingTopic']['id'])); ?></li>
				<li class="divider"></li>
				<li><?php echo $this->Html->link(__('Send for Approval'), '#makerchecker',array('data-toggle'=>'modal')); ?></li>
				<li><?php echo $this->Form->postLink(__('Email Record'), array('controller'=>'message','action' => 'add', $meetingTopic['MeetingTopic']['id']),array('class'=>''), __('Are you sure ?', $meetingTopic['MeetingTopic']['id'])); ?></li>
			</ul>
		</div>
		</td>
		<td width="50"><?php echo h($meetingTopic['MeetingTopic']['sr_no']); ?>&nbsp;</td>
		<td>
		...
		<td width="60">
			...
			<?php } ?>&nbsp;</td>
		<td><?php echo h($meetingTopic['MeetingTopic']['record_status']); ?>&nbsp;</td>
		<td>
			<?php echo $this->Html->link($meetingTopic['StatusUserId']['name'], array('controller' => 'users', 'action' => 'view', $meetingTopic['StatusUserId']['id'])); ?>
		</td>
		<td>
			<?php echo $this->Html->link($meetingTopic['BranchIds']['name'], array('controller' => 'branches', 'action' => 'view', $meetingTopic['BranchIds']['id'])); ?>
		</td>
		<td>
			<?php echo $this->Html->link($meetingTopic['DepartmentIds']['name'], array('controller' => 'departments', 'action' => 'view', $meetingTopic['DepartmentIds']['id'])); ?>
		</td>
		<td>
			<?php echo $this->Html->link($meetingTopic['ApprovedBy']['name'], array('controller' => 'employees', 'action' => 'view', $meetingTopic['ApprovedBy']['id'])); ?>
		</td>
		<td>
			<?php echo $this->Html->link($meetingTopic['PreparedBy']['name'], array('controller' => 'employees', 'action' => 'view', $meetingTopic['PreparedBy']['id'])); ?>
		</td>
		<td><?php echo h($meetingTopic['MeetingTopic']['division_id']); ?>&nbsp;</td>
		<td>
			<?php echo $this->Html->link($meetingTopic['Company']['name'], array('controller' => 'companies', 'action' => 'view', $meetingTopic['Company']['id'])); ?>
		</td>
	</tr>
<?php endforeach; ?>
<?php }else{ ?>
...
<?php echo $this->Js->event('click',$this->Js->request(array('action' => 'add', 'ajax'),array('async' => true, 'update' => '#meetingTopics_ajax')));?>
...
<?php echo $this->Form->create('meetingTopics',array('action'=>'report','target'=>'_blank','class'=>'no-padding no-margin no-background zero-height'));?>
