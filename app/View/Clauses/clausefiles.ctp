<?php echo "<h4>&nbsp;&nbsp;" . __($this->request->params['pass'][1]) . "<small> ". $this->Html->link('Upload file with approval',array('controller'=>'evidences','action'=>'lists','model'=>$this->request->params['controller'],'record'=>$this->request->params['pass'][0],'record_type'=>$this->request->params['pass'][1]),array('class'=>'pull-right btn btn-xs btn-warning')) ."</small></h4>"; ?>
...
<div class="row" id="dashboard_files_div">
  <div class="col-md-12">
    ...
    <?php if ($this->Session->read('User.is_mr') == true) { ?>
    <?php
    $filesData['action'] = str_replace(' ', '', $filesData['action']);
    $filesData['action'] = strtolower($filesData['action']);
          ...
          if ($this->Session->read('User.is_mr') == true)echo $this->Upload->edit('upload', "clauses/".$this->request->params['pass'][0] ."/". $this->request->params['pass'][1],false);
          else echo $this->Upload->view('upload', "clauses/".$this->request->params['pass'][0] ."/". $this->request->params['pass'][1]);
