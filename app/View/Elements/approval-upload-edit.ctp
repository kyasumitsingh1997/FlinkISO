<?php echo "<h4>&nbsp;&nbsp;" . __("Evidence Files") . "<small> ". $this->Html->link('Upload file with approval',array('controller'=>'evidences','action'=>'lists'),array('class'=>'pull-right btn btn-xs btn-warning')) ."</small></h4>"; ?>
...
<?php echo $this->Form->create('Upload', array('role' => 'form', 'class' => 'form','id'=>$apprecordId)); ?>
<?php
echo $apprecordId;
	echo $this->Upload->edit('upload', $usersId . '/' . $approvalSystemTableId['SystemTable']['id'] . '/' . $apprecordId,false);
	echo $this->Form->end();
?>
