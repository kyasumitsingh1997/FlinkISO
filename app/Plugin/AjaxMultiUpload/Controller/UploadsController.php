...
class UploadsController extends AjaxMultiUploadAppController {
	...
	public function upload($dir=null) {
		...
		$id = $this->_upload_add($result['details']['filename'],$result['details']['ext'],$result['details']['message'],$get_dir);		
		$this->set("result", htmlspecialchars(json_encode($result), ENT_NOQUOTES));
		
		$this->Session->setFlash(__('Document added successfully. Please add furture details.'));
  		$this->redirect(array('controller'=>'file_uploads','action' => 'view',$id));
