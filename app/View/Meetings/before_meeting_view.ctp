...
<table class="table table-responsive" style="margin-top:0px;">
    ...
    <?php if($meeting['Meeting']['meeting_type'] == 1){ ?> 
        ...
        <?php } ?>
        <tr><td><?php echo __('External Location'); ?></td>
            <td>
...
<div class="row">
    ...
    <?php if($meeting['Meeting']['meeting_type'] == 0){ ?> 
        ...
        <div id="meetingTopics_ajax<?php echo $i; ?>">
            <div class="row">
                <div class="panel panel-default">
                    <div class="panel-heading">Agenda <span class="text-danger glyphicon glyphicon-remove danger pull-right" style="font-size:20px;background:none"type="button" onclick='removeAgendaDiv(<?php echo $i; ?>)'></span></div>
                    <div class="panel-body">
                        <fieldset>

                            <div class="col-md-12"><?php echo $this->Form->input('MeetingTopic.' . $i . '.topic', array('label' => __('Topic'), 'value' => $meetingTopic['MeetingTopic']['title'])); ?>
                                <?php echo $this->Form->hidden('MeetingTopic.' . $i . '.topic_text', array('default'=>$meetingTopic['MeetingTopic']['topic_text'], 'value' => $meetingTopic['MeetingTopic']['current_status'])); ?>
                            </div>
                            <div class="col-md-6"><?php echo $this->Form->input('MeetingTopic.' . $i . '.current_status', array('default'=>$meetingTopic['MeetingTopic']['topic_text'], 'label' => __('Current Status'), 'value' => $meetingTopic['MeetingTopic']['current_status'])); ?></div>
...
<div class="row">
    <div class="col-md-12"><?php echo $this->Form->input('meeting_status', array('type'=>'radio', 'default'=>$meeting['Meeting']['meeting_status'], 'options'=>$meeting_statuses, 'label' => __('Meeting Status'))); ?></div>
</div>
<script>
    $("[name*='t_date']").datepicker({
        changeMonth: true,
        changeYear: true,
        format: 'yyyy-mm-dd',
      autoclose:true,
    ...
    $("[name*='t_date']").datepicker( "setDate" , "<?php echo date('Y-m-d',strtotime('+7 days'))?>" );
</script>
...
<script>
      ...
      autoclose:true,
        ...
        onClose: function (dateText, inst) {
            if (endDateTextBox.val() != '') {
                ...
                $("[name*=t_date]").datetimepicker("option", "minDate", testStartDate);
            } else {

                $("[name*=t_date]").datetimepicker("option", "minDate", dateText);
    ...
    $("[name*=t_date]").datetimepicker("option", "minDate", startDateTextBox.datetimepicker('getDate'));
</script>
