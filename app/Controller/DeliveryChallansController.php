...
class DeliveryChallansController extends AppController {
    ...
    public function view($id = null) {
        ...
        foreach ($deliveryChallanDetails as $deliveryChallanDetail) {
            if($deliveryChallanDetail['material_qc_required'] == 0){
                $qc = $this->MaterialQualityCheckDetail->find('all',array(
                    'conditions'=>array(
                        'MaterialQualityCheckDetail.delivery_challan_detail_id'=>$deliveryChallanDetail['DeliveryChallanDetail']['id'])));
                
                // get approvals
                $this->loadModel('Approval');
                foreach ($qc as $q) {
                    $approvals = $this->Approval->find('all',array('conditions'=>array('Approval.model_name'=>'MaterialQualityCheckDetail','record'=>$q['MaterialQualityCheckDetail']['id'])));
                    $q['Approval'] = $approvals;
                    $newQc[] = $q;
                }
                
                $deliveryChallanDetail['QcSteps'] = $newQc;
                $newDcDetails[] = $deliveryChallanDetail;
            }
        }
        $this->set('deliveryChallanDetails', $newDcDetails);
        return array('details'=>$newDcDetails,'challan'=>$deliveryChallan);        
    }
    ...
    public function add_ajax() {
        ...
        if ($this->request->is('post')) {
            ...
            $this->request->data['DeliveryChallan']['type']= 0;
            $this->request->data['DeliveryChallan']['challan_details'] = 'NIL';
            ...
            if ($this->DeliveryChallan->save($this->request->data)) {
                ...
                foreach ($this->request->data['delivery_challan_details'] as $value) {
                    ...
                    if ($value['material_id'] != '') {
                        if($value['material_qc_required'] != 1){
                            ...
                            $newData['purchase_order_id'] = $value['purchase_order_id'];
                            $newData['delivery_challan_id'] = $this->DeliveryChallan->id;
                            $newData['delivery_challan_detail_id'] = $this->DeliveryChallanDetail->id;
                            $newData['challan_date'] = $this->request->data['DeliveryChallan']['challan_date'];
                            $newData['material_id'] = $value['material_id'];
                            $newData['quantity_supplied'] = $value['quantity'];
                            $newData['quantity_accepted'] = $value['quantity_received'];
                    ...
                    if (($value['material_id'] != '') && ($purchaseOrder['PurchaseOrder']['type'] == 1) && ($value['material_qc_required'] != 1)) {
                        ...
                        // update stock status                        
                        $this->_update_stocks($value['material_id'], $stockData['quantity']);
        ...
        if($this->request->params['pass'][0]){            
            $purchaseOrders = $this->DeliveryChallan->PurchaseOrder->find('list', array('conditions' => array(
                'PurchaseOrder.id'=> $this->request->params['pass'][0],
                'PurchaseOrder.soft_delete' => 0)));
        }else{
        ...
        $num  = $this->generate_cp_number('DeliveryChallan', 'DC', 'challan_number');
        $this->set('dcNumber',$num);
    }
    ...
    public function get_purchase_order($id = null) {
        if ($id != -1) {
            ...
            // if($this->request->params['pass'][0]){
            $getDCs = $this->DeliveryChallan->find('all',array('conditions'=>array('DeliveryChallan.purchase_order_id'=>$id)));
                foreach ($getDCs as $challans) {
                    foreach ($challans['DeliveryChallanDetail'] as $challan) {
                        if($challan['product_id']){
                            $received[$challan['product_id']][] = $challan['quantity_received'];
                        }
                        if($challan['device_id']){
                            $received[$challan['device_id']][] = $challan['quantity_received'];
                        }
                        if($challan['material_id']){
                            $received[$challan['material_id']][] = $challan['quantity_received'];
                        }
    ...
    public function edit($id = null) {
        ...
        // block adding qc
        $dc = $this->DeliveryChallan->find('first',array('conditions'=>array('DeliveryChallan.id'=>$id)));
        $this->loadModel('DeliveryChallanDetail');
        foreach ($dc['DeliveryChallanDetail'] as $dcDetails) {
            debug($dcDetails);
            $newDcDetails = $dcDetails;
            $newDcDetails['publish'] = 0;
            debug($newDcDetails);
            $this->DeliveryChallanDetail->create();
            $this->DeliveryChallanDetail->save($newDcDetails,false);
        }
        $newDc = $dc['DeliveryChallan'];
        $newDc['publish'] = 0;
        $this->DeliveryChallan->create();
        $this->DeliveryChallan->save($newDc,false);
            ...
            if ($this->DeliveryChallan->save($this->request->data)) {
                ...
                foreach ($this->request->data['delivery_challan_details'] as $value) {
                    ...
                    if (($value['material_id'] != '') && ($purchaseOrder['PurchaseOrder']['type'] == 1) && ($value['material_qc_required'] != 1)) {
                        ...
                        $stockData['modified_by'] = $this->Session->read('User.id');
                        $this->Stock->save($stockData, false);
                        $this->_update_stocks($value['material_id']);
                    }
                }
                $this->Session->setFlash(__('The delivery challan has been saved'));
    ...
    public function get_delivered_material_qc() {
        ...
        $this->paginate = array('limit' => 10,
            'fields' => array(
                'DeliveryChallanDetail.id','DeliveryChallanDetail.delivery_challan_id', 'DeliveryChallanDetail.material_id', 'DeliveryChallanDetail.material_qc_required', 'DeliveryChallanDetail.quantity_received', 'Material.name', 'PurchaseOrder.title', 'DeliveryChallan.challan_date', 'DeliveryChallan.challan_number'
    ...
    public function set_pos(){
        ...
        $evaluations = $this->SupplierEvaluationReevaluation->find('all',array('recursive'=>-1));
        foreach ($evaluations as $evaluation) {
            $dc = $this->DeliveryChallan->find('first',array(
                'recursive'=>-1,
                'conditions'=>array('DeliveryChallan.id'=>$evaluation['SupplierEvaluationReevaluation']['delivery_challan_id'])));
            
            $newEval = $evaluation['SupplierEvaluationReevaluation'];
            $newEval['purchase_order_id'] = $dc['DeliveryChallan']['purchase_order_id'];
            
            $this->SupplierEvaluationReevaluation->create();
            $this->SupplierEvaluationReevaluation->save($newEval,false);
