			...
			<div class="row">
				<div class="col-md-4 pull-right">
					<div class="input-group pull-left">
						<?php echo $this->Form->create('EnvActivity',array('role'=>'form','class'=>'no-padding no-margin','id'=>'search-form','action'=>'search')); ?>
						...
						<?php echo $this->Form->input(__('search'),array('label'=>false,'placeholder'=>'search','role'=>'form','class'=>'form-control','action'=>'search')); ?>
							...
							<div class="input-group-btn">
								...
								<ul class="dropdown-menu pull-right">
									...
									<li><?php echo $this->Js->submit('Submit',array('url'=>'search','div'=>false,'class'=>'btn btn-primary btn-success','update' => '#envActivities_ajax','type'=>'data','method'=>'post')); ?>
...
<?php foreach ($envActivities as $envActivity): ?>
	<tr>
<td width="15"><?php echo $this->Form->checkbox('rec_ids_'.$i,array('label'=>false,'value'=>$envActivity['EnvActivity']['id'],'multiple'=>'checkbox','class'=>'rec_ids')); ?></td>		<td class=" actions">
		<div class="btn-group">
		<button type="button" data-toggle="dropdown" class="dropdown-toggle btn  btn-sm btn-default "><i class="fa fa-wrench" aria-hidden="true"></i></button>
				...
				<li><?php echo $this->Html->link(__('View / Upload Evidance'), array('action' => 'view', $envActivity['EnvActivity']['id'])); ?></li>
				<li><?php echo $this->Html->link(__('Edit'), array('action' => 'edit', $envActivity['EnvActivity']['id'])); ?></li>
				<li><?php echo $this->Form->postLink(__('Delete'), array('action' => 'delete', $envActivity['EnvActivity']['id']),array('class'=>''), __('Are you sure you want to delete this record ?', $envActivity['EnvActivity']['id'])); ?></li>
				<li class="divider"></li>
				<li><?php echo $this->Html->link(__('Send for Approval'), '#makerchecker',array('data-toggle'=>'modal')); ?></li>
				<li><?php echo $this->Form->postLink(__('Email Record'), array('controller'=>'message','action' => 'add', $envActivity['EnvActivity']['id']),array('class'=>''), __('Are you sure ?', $envActivity['EnvActivity']['id'])); ?></li>
			</ul>
		</div>
		</td>
		<td width="50"><?php echo h($envActivity['EnvActivity']['sr_no']); ?>&nbsp;</td>
		...
		<td><?php echo h($envActivity['EnvActivity']['other_details']); ?>&nbsp;</td>
		...
		<td width="60">
			...
			<?php } ?>&nbsp;</td>
		<td><?php echo h($envActivity['EnvActivity']['record_status']); ?>&nbsp;</td>
		<td><?php echo h($envActivity['EnvActivity']['status_user_id']); ?>&nbsp;</td>
		<td>
			<?php echo $this->Html->link($envActivity['BranchIds']['name'], array('controller' => 'branches', 'action' => 'view', $envActivity['BranchIds']['id'])); ?>
		</td>
		<td>
			<?php echo $this->Html->link($envActivity['DepartmentIds']['name'], array('controller' => 'departments', 'action' => 'view', $envActivity['DepartmentIds']['id'])); ?>
		</td>
		<td>
			<?php echo $this->Html->link($envActivity['ApprovedBy']['name'], array('controller' => 'employees', 'action' => 'view', $envActivity['ApprovedBy']['id'])); ?>
		</td>
		<td>
			<?php echo $this->Html->link($envActivity['PreparedBy']['name'], array('controller' => 'employees', 'action' => 'view', $envActivity['PreparedBy']['id'])); ?>
		</td>
	</tr>
<?php endforeach; ?>
<?php }else{ ?>
...
<?php echo $this->Js->event('click',$this->Js->request(array('action' => 'index', 'ajax'),array('async' => true, 'update' => '#envActivities_ajax')));?>

<?php echo $this->Js->get('#addrecord');?>
<?php echo $this->Js->event('click',$this->Js->request(array('action' => 'add', 'ajax'),array('async' => true, 'update' => '#envActivities_ajax')));?>
...
<?php echo $this->Form->create('envActivities',array('action'=>'report','target'=>'_blank','class'=>'no-padding no-margin no-background zero-height'));?>
