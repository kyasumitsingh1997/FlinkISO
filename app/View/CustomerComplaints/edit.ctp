...
<div id="customerComplaints_ajax">
    ...
    $().ready(function() {
        ...
        $("#submit_id").click(function(){
            if($('#CustomerComplaintEditForm').valid()){
                $("#submit_id").prop("disabled",true);
                $("#submit-indicator").show();
                $("#CustomerComplaintEditForm").submit();
            }
        });
        ...
        $('#CustomerComplaintEditForm').validate({
            rules: {
...
function hideCapa(){
    if($('#CustomerComplaintAddToCapa0').is(':checked')){
        $('#capaDetail').addClass('text-disabled');
...
<div class="nav panel panel-default">
    <div class="customerComplaints form col-md-8">
        <h4><?php echo $this->element('breadcrumbs') . __('Edit Customer Complaint'); ?>
        ...
        <div class="row">
            ...
            <div class="col-md-6"><br /><strong>Type: </strong>Customer Complaint</div>
        ...
        <div class="row">
            ...
            <div class="col-md-12"><?php echo $this->Form->input('initial_action'); ?></div>
            ...
            <div id="capaMsg" class="col-md-12 alert alert-danger ">Already added to Capa please edit from CAPA</div>
            <div id ='capaDetail'>
                <div class="col-md-4">
                    <?php
                ...
                <?php if($capa){ ?> 
                <div class="col-md-4">
                    <?php echo $capa['CorrectivePreventiveAction']['number']; ?><br />
                    <?php echo ($capa['CorrectivePreventiveAction']['current_status']?'Close':'Open'); ?><br />
                    <?php echo $this->Html->link('Edit CAPA',array('controller'=>'corrective_preventive_actions','action'=>'edit',$capa['CorrectivePreventiveAction']['id']),array('class'=>'btn btn-xs btn-info')); ?>
                </div>
                <?php }?>
            ...
            <div class="col-md-6"><?php 
                // if($this->request->data['CustomerComplaint']['action_taken_date']=='')$this->request->data['CustomerComplaint']['action_taken_date'] = date('Y-m-d');
            echo $this->Form->input('action_taken_date'); 
            ?></div>
            <div class="col-md-6"><?php 
                // if($this->request->data['CustomerComplaint']['settled_date']=='')$this->request->data['CustomerComplaint']['settled_date'] = date('Y-m-d');
            echo $this->Form->input('settled_date'); 
            ?></div>
            <?php } ?>                    
            <?php if (!($this->Session->read('User.is_mr') == 0 or $this->Session->read('User.is_mr') == false )) { ?>
            ...
            echo "<br /><label>" . __('Current Status') . "</label>";
            echo $this->Form->input('current_status', array('label' => '', 'legend' => false, 'div'=>array('class'=>''), 'value' => false, 'options' => array('0' => 'Open', '1' => 'Close'), 'checked' => $this->data['CustomerComplaint']['current_status'], 'type' => 'radio'));
            ?>
        </div>
...
<?php if($this->data['CustomerComplaint']['action_taken_date'] == '1970-01-01'){ ?> 
    <script type="text/javascript">
        $("#CustomerComplaintActionTakenDate").val('');
    </script>
<?php }?>
<?php if($this->data['CustomerComplaint']['action_taken_date'] == '1970-01-01'){ ?> 
    <script type="text/javascript">
        $("#CustomerComplaintSettledDate").val('');
    </script>
<?php }?>
<script>
$("#CustomerComplaintTargetDate").datepicker({
    changeMonth: true,
    changeYear: true,
    format: 'yyyy-mm-dd',
    autoclose:true,
...
$("#CustomerComplaintActionTakenDate").datepicker({
    useCurrent: false,
    changeMonth: true,
    changeYear: true,
    format: 'yyyy-mm-dd',
...
$("#CustomerComplaintSettledDate").datepicker({
    changeMonth: true,
    changeYear: true,
    format: 'yyyy-mm-dd',
    autoclose:true,
...
$("#CustomerComplaintComplaintDate").datepicker({
 changeMonth: true,
 changeYear: true,
 format: 'yyyy-mm-dd',
 autoclose:true,
 ...
 onSelect: function(selectedDateTime) {
    $("#CustomerComplaintTargetDate").datepicker('option', 'minDate', $("#CustomerComplaintComplaintDate").datepicker('getDate'));
    $("#CustomerComplaintActionTakenDate").datepicker('option', 'minDate', $("#CustomerComplaintComplaintDate").datepicker('getDate'));
    $("#CustomerComplaintSettledDate").datepicker('option', 'minDate', $("#CustomerComplaintComplaintDate").datepicker('getDate'));

}
}).attr('readonly', 'readonly');

$("#CustomerComplaintComplaintDate").datepicker('option', 'maxDate', 0);
</script>
