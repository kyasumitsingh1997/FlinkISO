...
<h4><?php echo __('Edit Project Activity Requirement'); ?>		
		<?php echo $this->Html->link(__('List This Project'), array('controller'=>'projects', 'action' => 'view',$this->request->data['ProjectActivityRequirement']['project_id']),array('id'=>'list','class'=>'label btn-info')); ?>
		<?php echo $this->Html->link(__('List Milestones'), array('controller'=>'milestones', 'action' => 'view','milestone_id'=>$this->request->data['ProjectActivityRequirement']['milestone_id']),array('id'=>'list','class'=>'label btn-info')); ?>
		<?php echo $this->Html->link(__('List Activities'), array('controller'=>'activities' ,'action' => 'view','activity_id',$this->request->data['ProjectActivityRequirement']['activity_id']),array('id'=>'list','class'=>'label btn-info')); ?>
		<?php echo $this->Html->link(__('List All'), array('action' => 'index'),array('id'=>'list','class'=>'label btn-info')); ?>
		
		</h4>
<?php echo $this->Form->create('ProjectActivityRequirement',array('role'=>'form','class'=>'form')); ?>
<div class="row">
			<?php
		echo "<div class='col-md-4'>".$this->Form->input('project_id',array('style'=>'')) . '</div>'; 
		echo "<div class='col-md-4'>".$this->Form->input('milestone_id',array('style'=>'')) . '</div>'; 
		echo "<div class='col-md-4'>".$this->Form->input('project_activity_id',array('style'=>'')) . '</div>'; 
		echo "<div class='col-md-12'>".$this->Form->input('title') . '</div>'; 
		echo "<div class='col-md-12'>".$this->Form->input('details') . '</div>'; 
		echo "<div class='col-md-6'>".$this->Form->input('manpower') . '</div>'; 
		echo "<div class='col-md-12'>".$this->Form->input('manpower_Details') . '</div>'; 
		echo "<div class='col-md-12'>".$this->Form->input('infrastructure') . '</div>'; 
		echo "<div class='col-md-12'>".$this->Form->input('other') . '</div>'; 
...
<script>
    ...
    $().ready(function() {
        $('#ProjectActivityRequirementEditForm').validate();
        $("#submit-indicator").hide();
        $("#submit_id").click(function(){
            if($('#ProjectActivityRequirementEditForm').valid()){
                 $("#submit_id").prop("disabled",true);
                 $("#submit-indicator").show();
                $('#ProjectActivityRequirementEditForm').submit();
            }
