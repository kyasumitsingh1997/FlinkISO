...
<h4><?php echo __('Edit Continual Improvement'); ?>		
		<?php echo $this->Html->link(__('List'), array('action' => 'index'),array('id'=>'list','class'=>'label btn-info')); ?>
		
		</h4>
<?php echo $this->Form->create('ContinualImprovement',array('role'=>'form','class'=>'form')); ?>
<div class="row">
			<?php
		...
		echo "<div class='col-md-6'>".$this->Form->input('internal_audit_detail_id',array('style'=>'')) . '</div>'; 
...
<script>
    ...
    $().ready(function() {
        $('#ContinualImprovementEditForm').validate();
        $("#submit-indicator").hide();
        $("#submit_id").click(function(){
            if($('#ContinualImprovementEditForm').valid()){
                 $("#submit_id").prop("disabled",true);
                 $("#submit-indicator").show();
                $('#ContinualImprovementEditForm').submit();
            }
