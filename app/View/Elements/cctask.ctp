<?php
$statusOptions = array('0' => 'On going', '1' => 'Completed','2'=>'Not Started','3'=>'Cancelled');
    if (count($tasks)) {
        $i = 0;
...
setTimeout(fade_out, 10000);
    ...
    $("#customerComplaint_task_submit").bind('click',function (event){
...
<div id = "customerComplaint_task_ajax_1">
    <div id="customerComplaint_task_ajax">
        <div id="flashMessageCustom">
           <?php echo $this->Session->flash(); ?>
        </div>
        <?php echo $this->Form->create('TaskStatus', array('role' => 'form', 'class' => 'form', 'default' => false)); ?>
            <!-- <h3 class="panel-title">
                <?php echo $this->Html->link(__('customerComplaint Tasks Assigned To You'), array('controller' => 'tasks', 'action' => 'index')); ?>
            ...
            <table class="table table-condensed checklists">
                ...
                <?php } ?>
                    ...
                    <td>
                        <?php echo $this->Html->link($task['Task']['name'], array('controller' => 'tasks', 'action' => 'view', $task['Task']['id'])); ?> <br /><small><?php echo $task['CustomerComplaint']['name']; ?></small>
                    </td>
                    ...
                    <!-- <td> -->
                            <?php
                                ...
                                if ($task['TaskStatus']['task_performed'] > 0 && ($task['TaskStatus']['id'] != $editId)) {
                                    ...
                                    else echo "<td>". $statusOptions[$task['Task']['task_status']]."<br /><small>End Date : " . $task['Task']['end_date'] . "</small></td>";
                                    ...
                                    if($task['Task']['revised_due_date'] && $task['Task']['revised_due_date'] != '1970-01-01')echo "On going<br /><small>Revised Due Date : " . $task['Task']['revised_due_date'] . "</small>";
                                    else echo "<small>End Date : " . $task['Task']['end_date'] . "</small>";
                                }
                            ?>
                    <!-- </td> -->
                        <!-- <td><?php 
                    ...
                    <td><?php
                                if ($task['TaskStatus']['id']){
                                    echo $this->Js->link('<i class="fa fa-pencil-square-o" aria-hidden="true"></i>', 
                                        array('controller' => 'tasks', 'action' => 'get_cc_task', $task['TaskStatus']['id']), 
                                        array('escape' => false, 'update' => '#customerComplaint_task_ajax', 'async' => 'false'))
                            ?>
                    </td>
                    <td>
                            <div class="btn-group  pull-right">
                                <div id="customerComplaint_taskcountdiv<?php echo $task['TaskStatus']['id'];?>" class="btn-xs btn btn-primary"></div>                                            
                                <?php echo $this->Html->link('Files','#',array('id'=>'customerComplaint_taskfilesbtn'.$task['TaskStatus']['id'],'class'=>'btn btn-xs btn-info')); ?>    
                            </div>
                    
                    
                            <div id="customerComplaint_taskfilesdiv<?php echo $task['TaskStatus']['id'];?>"></div>                           
                            <script>
                                
                                    $('#customerComplaint_taskfilesbtn<?php echo $task["TaskStatus"]["id"];?>').click(function(){
                                        $('#customerComplaint_taskfilesdiv<?php echo $task["TaskStatus"]["id"];?>').load("<?php echo Router::url('/', true); ?>tasks/cc_task_ajax/<?php echo $task['TaskStatus']['id'] ?>/<?php echo $task['TaskStatus']['id']; ?>", function(response, status, xhr){});
                                });
                            </script>
                            <script>
                                    $().ready(function(){$('#customerComplaint_taskcountdiv<?php echo $task['TaskStatus']['id'];?>').load('<?php echo Router::url('/', true); ?>tasks/customerComplaint_task_ajax_file_count/<?php echo $task['TaskStatus']['id'] ?>', function(response, status, xhr){});});
                                    </script>
                ...
                <tr>
                    <td colspan='7'>
                            <?php
                                if ($i == 1)
                                    echo $this->Js->submit(__('Submit'), array('url' => array('controller' => 'tasks', 'action' => 'get_cc_task'), 'div' => false, 'class' => 'btn btn-primary btn-success', 'update' => '#customerComplaint_task_ajax_1', 'async' => 'false','id'=>'customerComplaint_task_submit'));
                            ?>
                    </td>
                </tr>
            </table>
...
<?php } else{ ?>
    <div id="customerComplaint_tasks_ajax" style="padding:10px">
