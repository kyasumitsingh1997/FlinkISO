		...
		<?php echo $this->Form->create(array('class'=>'no-padding no-margin no-background'));?>
			<table cellpadding="0" cellspacing="0" class="table table-bordered table table-striped table-hover">
				<tr>
					...
					<th><?php echo $this->Paginator->sort('reports','Reports Required'); ?></th>					
				</tr>
				<?php if($systemTables){ ?>
					<?php $x=0;
					...
					<script>
						$(document).ready(function() {$('#evi-<?php echo $x; ?>').editable({
                           ...
                           url:   '<?php echo Router::url('/', true);?><?php echo $this->request->params['controller']?>/inplace_edit_evidence',  
                           title: 'Change Status'
                        });});
					</script>
					<script>
						$(document).ready(function() {$('#app-<?php echo $x; ?>').editable({
                           ...
                           url:   '<?php echo Router::url('/', true);?><?php echo $this->request->params['controller']?>/inplace_edit_approval',  
                           title: 'Change Status'
                        });});
					</script>
					<script>
						$(document).ready(function() {$('#rep-<?php echo $x; ?>').editable({
                           type:  'text',
                           pk:    '<?php echo $systemTable['SystemTable']['id']; ?>',
                           name:  'data.SystemTable.reports',
                           url:   '<?php echo Router::url('/', true);?><?php echo $this->request->params['controller']?>/inplace_edit_reports',  
                           title: 'Change Status'
                        });});
					</script>
	<tr class="on_page_src">

		<td><?php echo h($systemTable['SystemTable']['name']); ?>&nbsp;<?php echo h($systemTable['SystemTable']['system_name']); ?></td>		
		<td><a href="#" id="evi-<?php echo $x; ?>"><?php echo ($systemTable['SystemTable']['evidence_required'])? "Yes" : "No"; ?></a>&nbsp;</td>
		<td><a href="#" id="app-<?php echo $x; ?>"><?php echo ($systemTable['SystemTable']['approvals_required'])? "Yes" : "No"; ?></a>&nbsp;</td>
		<td><a href="#" id="rep-<?php echo $x; ?>"><?php echo ($systemTable['SystemTable']['reports'])? "Yes" : "No"; ?></a>&nbsp;</td>
...
<?php echo $this->Form->end();?>
	...
	<div class="col-md-4"><p>
		...
		<?php foreach ($missing as $key => $value) {
			echo "<li>". $this->Html->link(Inflector::Humanize($value),array('action'=>'lists',$value)).  "</li>";
		}?>
		</ul>
	</p>
	</div>
