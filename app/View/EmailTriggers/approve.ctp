...
<h4><?php echo __('Approve Email Trigger'); ?>		
...
<div class="row">
		...
		echo "<div class='col-md-6'>".$this->Form->input('system_table',array()) . '</div>'; 
                  ?>
                                         ...
                                         <div class="col-md-6"><?php echo $this->Form->input('state_id', array('style' => 'width:100%', 'label' => __('State'), 'options' => $PublishedStateList)); ?></div>
                               
                <div class="col-md-6" ><?php echo $this->Form->input('city_id', array('style' => 'width:100%', 'label' => __('City'), 'options' => $PublishedCityList)); ?></div>
...
//    });
$("#submit-indicator").hide();
        $('#EmailTriggerApproveForm').validate();
        $("#submit_id").click(function(){
             if($('#EmailTriggerApproveForm').valid()){
                 $("#submit_id").prop("disabled",true);
                 $("#submit-indicator").show();
                 $('#EmailTriggerApproveForm').submit();
             }
        });
    $('#EmailTriggerStateId').change(function() {
            $("#EmailTriggerCityId").load('<?php echo Router::url('/', true); ?>branches/get_city_list/' + encodeURIComponent(this.value), function(response, status, xhr) {
                
                    $("#EmailTriggerCityId").html(response);
                    $('#EmailTriggerCityId').val(0).trigger('chosen:updated');
               
            });
        });
         $('#EmailTriggerCityId').change(function() {
           
            $("#EmailTriggerBranchId").load('<?php echo Router::url('/', true); ?>branches/get_branch_list/' + encodeURIComponent(this.value), function(response, status, xhr) {
                
                $("#EmailTriggerBranchId").html(response);
                $('#EmailTriggerBranchId').val(0).trigger('chosen:updated');
