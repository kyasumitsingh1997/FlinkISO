...
class DeviceMaintenancesController extends AppController {
    ...
    public function pending_tasks() {
        ...
        $conditions = $this->_check_request();
        $conditions = array($conditions,'DeviceMaintenance.next_maintanence_date >' =>date('Y-m-d'));
        $this->paginate = array('order' => array('DeviceMaintenance.sr_no' => 'DESC'), 'conditions' => array($conditions));
    ...
    public function send_reminder($id = null){
        $cc = $this->DeviceMaintenance->find('first',array('recursive'=>0, 'conditions'=>array('DeviceMaintenance.id'=>$this->request->params['pass'][0])));
        ...
        $employee = $this->Employee->find('first',array(
            ...
            'conditions'=>array('Employee.id'=>$cc['DeviceMaintenance']['employee_id'])));
        ...
        if($cc && $email){
            $send_message = "Pending Device Maintenance";
            $body = "<p>You have pending Device Maintenance to address. Please login to FlinkISO and add details</p>";
            try{
    ...
    public function device_maintainance_plan($year = null){
        ...
        $devices = $this->DeviceMaintenance->Device->find('all',array(
            'recursive'=>-1,
            'fields'=>array('Device.id','Device.name','Device.number','Device.serial','Device.employee_id','maintenance_required'),
            'conditions'=>array('Device.publish'=>1,'Device.soft_delete'=>0,'Device.maintenance_required'=>1)));

        
            for($i = 1; $i <= 12; $i++){
            // $audit_number = 0;
            $device_maintainance = 0;
            foreach ($devices as $device) {
                
                $device_maintainance = 0;
                $device_maintainance = $this->DeviceMaintenance->find('count',array('conditions'=>array(
                    'DeviceMaintenance.device_id'=>$device['Device']['id'],                    
                    'OR'=>array(
                        array('MONTH(DeviceMaintenance.maintenance_performed_date)' => $i,'YEAR(DeviceMaintenance.maintenance_performed_date)' => $this->request->params['named']['year']),
                        array('MONTH(DeviceMaintenance.next_maintanence_date)' => $i,'YEAR(DeviceMaintenance.next_maintanence_date)' => $this->request->params['named']['year'])
                    )                    
                )));
                
                $data[$device['Device']['name']][] = array($i,$device_maintainance);
