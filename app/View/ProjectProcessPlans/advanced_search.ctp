...
<?php foreach ($projectProcessPlans as $projectProcessPlan): ?>
	<tr>
<td width="15"><?php echo $this->Form->checkbox('rec_ids_'.$i,array('label'=>false,'value'=>$projectProcessPlan['ProjectProcessPlan']['id'],'multiple'=>'checkbox','class'=>'rec_ids')); ?></td>		<td class=" actions">
		<div class="btn-group">
		<button type="button" data-toggle="dropdown" class="dropdown-toggle btn  btn-sm btn-default "><span class=" glyphicon glyphicon-wrench"></span></button>
				...
				<li><?php echo $this->Html->link(__('View / Upload Evidance'), array('action' => 'view', $projectProcessPlan['ProjectProcessPlan']['id'])); ?></li>
...
<?php echo $this->Form->create('projectProcessPlans',array('action'=>'report','target'=>'_blank','class'=>'no-padding no-margin no-background zero-height'));?>
