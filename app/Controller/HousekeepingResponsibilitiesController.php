...
class HousekeepingResponsibilitiesController extends AppController {
    ...
    public function approve($id = null, $approvalId = null) {
     ...
     public function housekeeping_ajax(){
        $this->loadModel('SystemTable');
        $this->loadModel('FileUpload');
        $houskeeping_task_approval_system_table = $this->SystemTable->find('first',array('conditions'=>array('SystemTable.system_name'=>'approvals')));
        $houskeeping_task_approval_files = $this->FileUpload->find('all',array('conditions'=>array('FileUpload.archived'=>0,'FileUpload.record'=>$this->request->params['pass'][0],'FileUpload.system_table_id'=>$houskeeping_task_approval_system_table['SystemTable']['id'])));
        $this->set('houskeeping_task_approval_files',$houskeeping_task_approval_files);        
    }

    public function housekeeping_ajax_file_count(){
        $this->loadModel('SystemTable');
        $this->loadModel('FileUpload');
        $houskeeping_task_approval_system_table = $this->SystemTable->find('first',array('conditions'=>array('SystemTable.system_name'=>'approvals')));
        $houskeeping_task_approval_files = $this->FileUpload->find('count',array('conditions'=>array('FileUpload.archived'=>0,'FileUpload.record'=>$this->request->params['pass'][0],'FileUpload.system_table_id'=>$houskeeping_task_approval_system_table['SystemTable']['id'])));
        $this->set('houskeeping_task_approval_files_count',$houskeeping_task_approval_files);
    }
