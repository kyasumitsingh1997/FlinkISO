...
echo $this->fetch('css');
echo $this->Html->script(array('chosen.min','jquery.validate.min', 'jquery-form.min'));
...
<div class="panel">
    <div class="panel-heading">
        <h2><small><?php echo $this->Html->link('Users',array('action'=>'index')); ?> / </small> Set User Access</h2>
    </div>
...
<script>
    $.validator.setDefaults({
        ...
        errorPlacement: function(error, element) {
            if ($(element).attr('name') == 'data[UserAccessControl][division_id]'){   
        ...
        $().ready(function() {
            ...
            $('#UserAccessControlAddAjaxForm').validate({
                rules: {
