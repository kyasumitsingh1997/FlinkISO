			...
			<div class="row">
				<div class="col-md-4 pull-right">
					<div class="input-group pull-left">
						<?php echo $this->Form->create('IncidentInvestigator',array('role'=>'form','class'=>'no-padding no-margin','id'=>'search-form','action'=>'search')); ?>
						...
						<?php echo $this->Form->input(__('search'),array('label'=>false,'placeholder'=>'search','role'=>'form','class'=>'form-control','action'=>'search')); ?>
							...
							<div class="input-group-btn">
								...
								<ul class="dropdown-menu pull-right">
									<li class="search-dropdown"><?php echo $this->Form->input('search_field',array('multiple'=>'checkbox','options'=>array("sr_no"=>"Sr No","name"=>"Name","address"=>"Address","phone"=>"Phone","age"=>"Age","gender"=>"Gender","record_status"=>"Record Status","branchid"=>"Branchid","departmentid"=>"Departmentid","approved_by"=>"Approved By","prepared_by"=>"Prepared By")));?></li>		
									<li><?php echo $this->Js->submit('Submit',array('url'=>'search','div'=>false,'class'=>'btn btn-primary btn-success','update' => '#incidentInvestigators_ajax','type'=>'data','method'=>'post')); ?>
;									<?php echo $this->Form->end(); ?></li>
				...
				<div class="col-md-8">
					<h4><?php echo __('Incident Investigators'); ?>
...
<?php foreach ($incidentInvestigators as $incidentInvestigator): ?>
	<tr class="on_page_src">
                    <td width="15"><?php echo $this->Form->checkbox('rec_ids_'.$i,array('label'=>false,'value'=>$incidentInvestigator['IncidentInvestigator']['id'],'multiple'=>'checkbox','class'=>'rec_ids')); ?></td>		<td class=" actions">
		<div class="btn-group">
		<button type="button" data-toggle="dropdown" class="dropdown-toggle btn  btn-sm btn-default "><i class="fa fa-wrench" aria-hidden="true"></i></button>
				...
				<li><?php echo $this->Html->link(__('View / Upload Evidance'), array('action' => 'view', $incidentInvestigator['IncidentInvestigator']['id'])); ?></li>
				<li><?php echo $this->Html->link(__('Edit'), array('action' => 'edit', $incidentInvestigator['IncidentInvestigator']['id'])); ?></li>
				<li><?php echo $this->Form->postLink(__('Delete'), array('action' => 'delete', $incidentInvestigator['IncidentInvestigator']['id']),array('class'=>''), __('Are you sure you want to delete this record ?', $incidentInvestigator['IncidentInvestigator']['id'])); ?></li>
				<li class="divider"></li>
				<li><?php echo $this->Html->link(__('Send for Approval'), '#makerchecker',array('data-toggle'=>'modal')); ?></li>
				<li><?php echo $this->Form->postLink(__('Email Record'), array('controller'=>'message','action' => 'add', $incidentInvestigator['IncidentInvestigator']['id']),array('class'=>''), __('Are you sure ?', $incidentInvestigator['IncidentInvestigator']['id'])); ?></li>
			</ul>
		</div>
		</td>
		...
		<td>
			<?php echo $this->Html->link($incidentInvestigator['BranchIds']['name'], array('controller' => 'branches', 'action' => 'view', $incidentInvestigator['BranchIds']['id'])); ?>
		</td>
		<td>
			<?php echo $this->Html->link($incidentInvestigator['DepartmentIds']['name'], array('controller' => 'departments', 'action' => 'view', $incidentInvestigator['DepartmentIds']['id'])); ?>
		</td>
		<td>
			<?php echo $this->Html->link($incidentInvestigator['ApprovedBy']['name'], array('controller' => 'employees', 'action' => 'view', $incidentInvestigator['ApprovedBy']['id'])); ?>
		</td>
		<td>
			<?php echo $this->Html->link($incidentInvestigator['PreparedBy']['name'], array('controller' => 'employees', 'action' => 'view', $incidentInvestigator['PreparedBy']['id'])); ?>
		</td>
		<td>
			<?php echo $this->Html->link($incidentInvestigator['Company']['name'], array('controller' => 'companies', 'action' => 'view', $incidentInvestigator['Company']['id'])); ?>
		</td>
	</tr>
<?php endforeach; ?>
<?php }else{ ?>
...
<?php echo $this->Form->end();?>			
			...
			'update' => '#incidentInvestigators_ajax',
			'evalScripts' => true,
...
<?php echo $this->Js->event('click',$this->Js->request(array('action' => 'add', 'ajax'),array('async' => true, 'update' => '#incidentInvestigators_ajax')));?>
...
<?php echo $this->Form->create('incidentInvestigators',array('action'=>'report','target'=>'_blank','class'=>'no-padding no-margin no-background zero-height'));?>
