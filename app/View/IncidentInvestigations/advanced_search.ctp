...
<?php foreach ($incidentInvestigations as $incidentInvestigation): ?>
	<tr class="on_page_src">
                    <td width="15"><?php echo $this->Form->checkbox('rec_ids_'.$i,array('label'=>false,'value'=>$incidentInvestigation['IncidentInvestigation']['id'],'multiple'=>'checkbox','class'=>'rec_ids')); ?></td>		<td class=" actions">
		<div class="btn-group">
		<button type="button" data-toggle="dropdown" class="dropdown-toggle btn  btn-sm btn-default "><i class="fa fa-wrench" aria-hidden="true"></i></button>
				...
				<li><?php echo $this->Html->link(__('View / Upload Evidance'), array('action' => 'view', $incidentInvestigation['IncidentInvestigation']['id'])); ?></li>
...
<?php echo $this->Form->create('incidentInvestigations',array('action'=>'report','target'=>'_blank','class'=>'no-padding no-margin no-background zero-height'));?>
