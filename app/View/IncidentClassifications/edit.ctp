...
<h4><?php echo __('Edit Incident Classification'); ?>		
		<?php echo $this->Html->link(__('List'), array('action' => 'index'),array('id'=>'list','class'=>'label btn-info')); ?>
		
		</h4>
<?php echo $this->Form->create('IncidentClassification',array('role'=>'form','class'=>'form')); ?>
<div class="row">
			<?php
...
<script>
    ...
    $().ready(function() {
        $('#IncidentClassificationEditForm').validate();
        $("#submit-indicator").hide();
        $("#submit_id").click(function(){
            if($('#IncidentClassificationEditForm').valid()){
                 $("#submit_id").prop("disabled",true);
                 $("#submit-indicator").show();
                $('#IncidentClassificationEditForm').submit();
            }
