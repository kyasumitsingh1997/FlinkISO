<div id="rejectionDetails_ajax">
<?php echo $this->Session->flash();?>	
<div class="nav panel panel-default">
<div class="rejectionDetails form col-md-8">
<h4><?php echo __('View Rejection Detail'); ?>		<?php echo $this->Html->link(__('List'), array('action' => 'index'),array('id'=>'list','class'=>'label btn-info')); ?>
		<?php echo $this->Html->link(__('Download PDF'), array('action' => 'view',$this->request->params['pass'][0].'.pdf'),array('id'=>'pdf','class'=>'label btn-info')); ?>
		<?php echo $this->Html->link(__('Edit'), '#edit',array('id'=>'edit','class'=>'label btn-info','data-toggle'=>'modal')); ?>
		<?php echo $this->Html->image('indicator.gif', array('id' => 'busy-indicator')); ?>
		</h4>

<table class="table table-responsive">
		<tr><td><?php echo __('Production Rejection'); ?></td>
		<td>
			<?php echo $this->Html->link($rejectionDetail['ProductionRejection']['name'], array('controller' => 'production_rejections', 'action' => 'view', $rejectionDetail['ProductionRejection']['id'])); ?>
			&nbsp;
		</td></tr>
		<tr><td><?php echo __('Value Driver'); ?></td>
		<td>
			<?php echo $this->Html->link($rejectionDetail['ValueDriver']['name'], array('controller' => 'value_drivers', 'action' => 'view', $rejectionDetail['ValueDriver']['id'])); ?>
			&nbsp;
		</td></tr>
		<tr><td><?php echo __('Defect Type'); ?></td>
		<td>
			<?php echo $this->Html->link($rejectionDetail['DefectType']['name'], array('controller' => 'defect_types', 'action' => 'view', $rejectionDetail['DefectType']['id'])); ?>
			&nbsp;
		</td></tr>
		<tr><td><?php echo __('Performance Indicator'); ?></td>
		<td>
			<?php echo $this->Html->link($rejectionDetail['PerformanceIndicator']['name'], array('controller' => 'performance_indicators', 'action' => 'view', $rejectionDetail['PerformanceIndicator']['id'])); ?>
			&nbsp;
		</td></tr>
		<tr><td><?php echo __('Prepared By'); ?></td>

	<td><?php echo h($rejectionDetail['ApprovedBy']['name']); ?>&nbsp;</td></tr>
		<tr><td><?php echo __('Approved By'); ?></td>

	<td><?php echo h($rejectionDetail['ApprovedBy']['name']); ?>&nbsp;</td></tr>
		<tr><td><?php echo __('Publish'); ?></td>

		<td>
			<?php if($rejectionDetail['RejectionDetail']['publish'] == 1) { ?>
			<span class="fa fa-check"></span>
			<?php } else { ?>
			<span class="fa fa-ban"></span>
			<?php } ?>&nbsp;</td>
&nbsp;</td></tr>
		<tr><td><?php echo __('Soft Delete'); ?></td>

		<td>
			<?php if($rejectionDetail['RejectionDetail']['soft_delete'] == 1) { ?>
			<span class="fa fa-check"></span>
			<?php } else { ?>
			<span class="fa fa-ban"></span>
			<?php } ?>&nbsp;</td>
&nbsp;</td></tr>
</table>
<?php echo ('upload-edit', array('usersId' => rejectionDetail['RejectionDetail']['created_by'], 'recordId' => rejectionDetail['RejectionDetail']['id'])); ?>
;
</div>
<div class="col-md-4">
	<p><?php echo $this->element('helps'); ?></p>
</div>
</div>
<?php echo $this->Js->get('#list');?>
<?php echo $this->Js->event('click',$this->Js->request(array('action' => 'index', 'ajax'),array('async' => true, 'update' => '#rejectionDetails_ajax')));?>

<?php echo $this->Js->get('#edit');?>
<?php echo $this->Js->event('click',$this->Js->request(array('action' => 'edit',$rejectionDetail['RejectionDetail']['id'] ,'ajax'),array('async' => true, 'update' => '#rejectionDetails_ajax')));?>


<?php echo $this->Js->writeBuffer();?>

</div>
<script>$.ajaxSetup({beforeSend:function(){$("#busy-indicator").show();},complete:function(){$("#busy-indicator").hide();}});</script>
