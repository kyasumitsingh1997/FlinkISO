			...
			<div class="row">
				<div class="col-md-4 pull-right">
					<div class="input-group pull-left">
						<?php echo $this->Form->create('FileView',array('role'=>'form','class'=>'no-padding no-margin','id'=>'search-form','action'=>'search')); ?>
						...
						<?php echo $this->Form->input(__('search'),array('label'=>false,'placeholder'=>'search','role'=>'form','class'=>'form-control','action'=>'search')); ?>
							...
							<div class="input-group-btn">
								...
								<ul class="dropdown-menu pull-right">
									...
									<li><?php echo $this->Js->submit('Submit',array('url'=>'search','div'=>false,'class'=>'btn btn-primary btn-success','update' => '#fileViews_ajax','type'=>'data','method'=>'post')); ?>
...
<?php foreach ($fileViews as $fileView): ?>
	<tr>
<td width="15"><?php echo $this->Form->checkbox('rec_ids_'.$i,array('label'=>false,'value'=>$fileView['FileView']['id'],'multiple'=>'checkbox','class'=>'rec_ids')); ?></td>		<td class=" actions">
		<div class="btn-group">
		<button type="button" data-toggle="dropdown" class="dropdown-toggle btn  btn-sm btn-default "><i class="fa fa-wrench" aria-hidden="true"></i></button>
				...
				<li><?php echo $this->Html->link(__('View / Upload Evidance'), array('action' => 'view', $fileView['FileView']['id'])); ?></li>
				<li><?php echo $this->Html->link(__('Edit'), array('action' => 'edit', $fileView['FileView']['id'])); ?></li>
				<li><?php echo $this->Form->postLink(__('Delete'), array('action' => 'delete', $fileView['FileView']['id']),array('class'=>''), __('Are you sure you want to delete this record ?', $fileView['FileView']['id'])); ?></li>
				<li class="divider"></li>
				<li><?php echo $this->Html->link(__('Send for Approval'), '#makerchecker',array('data-toggle'=>'modal')); ?></li>
				<li><?php echo $this->Form->postLink(__('Email Record'), array('controller'=>'message','action' => 'add', $fileView['FileView']['id']),array('class'=>''), __('Are you sure ?', $fileView['FileView']['id'])); ?></li>
			</ul>
		</div>
		</td>
		<td width="50"><?php echo h($fileView['FileView']['sr_no']); ?>&nbsp;</td>
		<td>
		...
		<td width="60">
			...
			<?php } ?>&nbsp;</td>
		<td><?php echo h($fileView['FileView']['record_status']); ?>&nbsp;</td>
		<td>
			<?php echo $this->Html->link($fileView['BranchIds']['name'], array('controller' => 'branches', 'action' => 'view', $fileView['BranchIds']['id'])); ?>
		</td>
		<td>
			<?php echo $this->Html->link($fileView['DepartmentIds']['name'], array('controller' => 'departments', 'action' => 'view', $fileView['DepartmentIds']['id'])); ?>
		</td>
		<td>
			<?php echo $this->Html->link($fileView['ApprovedBy']['name'], array('controller' => 'employees', 'action' => 'view', $fileView['ApprovedBy']['id'])); ?>
		</td>
		<td>
			<?php echo $this->Html->link($fileView['PreparedBy']['name'], array('controller' => 'employees', 'action' => 'view', $fileView['PreparedBy']['id'])); ?>
		</td>
		<td>
			<?php echo $this->Html->link($fileView['Division']['name'], array('controller' => 'divisions', 'action' => 'view', $fileView['Division']['id'])); ?>
		</td>
		<td>
			<?php echo $this->Html->link($fileView['Company']['name'], array('controller' => 'companies', 'action' => 'view', $fileView['Company']['id'])); ?>
		</td>
	</tr>
<?php endforeach; ?>
<?php }else{ ?>
...
<?php echo $this->Js->event('click',$this->Js->request(array('action' => 'add', 'ajax'),array('async' => true, 'update' => '#fileViews_ajax')));?>
...
<?php echo $this->Form->create('fileViews',array('action'=>'report','target'=>'_blank','class'=>'no-padding no-margin no-background zero-height'));?>
