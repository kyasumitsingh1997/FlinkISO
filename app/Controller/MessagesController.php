...
class MessagesController extends AppController {
    ...
    public function inbox() {
        ...
        $this->paginate = array('MessageUserInbox' => array('order' => array('MessageUserInbox.id' => 'DESC'), 
            'group' => array('MessageUserInbox.trackingid'), 
            'conditions' => array('MessageUserInbox.status <>' => 2, 
        ...
        foreach ($data as $message) {
            ...
            $message_from = $this->Message->User->find('all', array('conditions' => array('User.id' => $message['CreatedBy']['id']), 'fields' => array('User.id', 'User.name', 'Branch.id', 'Branch.name')));
            $newdata[$i] = $message;
    ...
    public function inbox_dashboard(){     
        $newdata = false;   
       $this->loadModel('MessageUserInbox');
        $this->MessageUserInbox->recursive = 1;
        $this->paginate = array('MessageUserInbox' => array('limit'=>5, 'order' => array('MessageUserInbox.id' => 'DESC'), 'group' => array('MessageUserInbox.message_id'), 'conditions' => array('MessageUserInbox.status <>' => 2, 'MessageUserInbox.user_id' => $this->Session->read('User.id'))));
        $newData = array();
        $data = $this->paginate('MessageUserInbox');
        $i = 0;
        foreach ($data as $message) {
    ...
    public function sent() {
        ...
        $i = 0;
        $messages = $this->Message->MessageUserSent->find("all", array('order'=>array('Message.created'=>'DESC'), "conditions" => array(
    ...
    public function reply($id = null,$re = null) {
        if ($this->request->data) {
            if ($this->request->is('post')) {
                ...
                if ($this->Message->save($this->request->data, false)) {
                    ...
                    if($this->request->params['pass']['0'] == null )$this->redirect(array('action' => 'index'));
                    else $this->redirect(array('action' => 'inbox_dashboard'));
                } else {
                    $this->Session->setFlash(__('The message could not be saved. Please, try again.'));
                }
            }
        ...
        $findMessage = $this->Message->find('first',array('conditions'=>array('Message.id'=> $this->request->params['pass'][0])));
        
        $messageThread = $this->Message->find('all', array('order' => array('Message.id ASC'), 'conditions' => array('Message.id' => $this->request->params['pass'][0])));
        ...
        $gettrackingid = $this->Message->read(null, $this->request->params['pass'][0]);

        $userInboxid = $this->Message->find('all', array('conditions' => array('Message.id ' => $this->request->params['pass'][0])));
    ...
    public function delete_all($id = NULL, $parent_id = NULL) {
