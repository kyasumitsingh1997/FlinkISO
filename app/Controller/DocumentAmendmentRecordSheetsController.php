...
class DocumentAmendmentRecordSheetsController extends AppController {
    ...
    public function view($id = null) {
        ...
        $this->DocumentAmendmentRecordSheet->recursive = 1;
        ...
        if($documentAmendmentRecordSheet['DocumentAmendmentRecordSheet']['file_upload_id'] == -1 or $documentAmendmentRecordSheet['DocumentAmendmentRecordSheet']['file_upload_id'] == null){
            ...
            $this->document_revisions($documentAmendmentRecordSheet['DocumentAmendmentRecordSheet']['master_list_of_format']);
        }else{
            $firstDocument = $this->DocumentAmendmentRecordSheet->FileUpload->find('first',array('conditions'=>array(
                    'FileUpload.system_table_id' => $documentAmendmentRecordSheet['FileUpload']['system_table_id'],
                    'FileUpload.record' => $documentAmendmentRecordSheet['FileUpload']['record'],
                    'FileUpload.archived' => 0
                ),
                'order' => array('FileUpload.version' => 'DESC')
            ));
            
            $revisionHistorys = $this->DocumentAmendmentRecordSheet->FileUpload->find('all',array('conditions'=>array(
                    'FileUpload.system_table_id' => $documentAmendmentRecordSheet['FileUpload']['system_table_id'],
                    'FileUpload.record' => $documentAmendmentRecordSheet['FileUpload']['record'],                    
                    'FileUpload.archived' => 1
            )));

            $this->set(compact('revisionHistorys', 'firstDocument'));
            $this->document_revisions($documentAmendmentRecordSheet['DocumentAmendmentRecordSheet']['file_upload_id']);
        }

        // get files
        // $this->loadModel('FileUpload');
        
        // $files = $this->FileUpload->find('all',array('limit'=>5));        
    }
    ...
    public function edit($id = null) {
        ...
        $masterListOfFormats = $this->DocumentAmendmentRecordSheet->MasterListOfFormat->find('list', array('conditions' => array('MasterListOfFormat.publish' => 1, 'MasterListOfFormat.soft_delete' => 0, 'MasterListOfFormat.archived' => 0)));
        $this->set(compact('branches', 'departments', 'employees', 'customers', 'SuggestionForms', 'systemTables', 'masterListOfFormats'));
		$this->document_revisions($this->request->data['DocumentAmendmentRecordSheet']['master_list_of_format']);
    }
