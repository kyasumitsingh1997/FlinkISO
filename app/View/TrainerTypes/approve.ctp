...
<script>
    $().ready(function() {
        $('#TrainerTypeApproveForm').validate();
        $("#submit-indicator").hide();
        $("#submit_id").click(function(){
             if($('#TrainerTypeApproveForm').valid()){
                 $("#submit_id").prop("disabled",true);
                 $("#submit-indicator").show();
		 $("#TrainerTypeApproveForm").submit();
             }
...
<div id="trainerTypes_ajax">
    ...
    <div class="nav panel panel-default">
        <div class="trainerTypes form col-md-8">
            <h4><?php echo $this->element('breadcrumbs') . __('Approve Trainer Type'); ?>
                <?php echo $this->Html->link(__('List'), array('action' => 'index'), array('id' => 'list', 'class' => 'label btn-info')); ?>
          <?php echo $this->Html->link(__('Download PDF'), array('action' => 'view',$this->request->params['pass'][0].'.pdf'), array('id' => 'pdf', 'class' => 'label btn-info')); ?>

            </h4>
            <?php echo $this->Form->create('TrainerType', array('role' => 'form', 'class' => 'form')); ?>
