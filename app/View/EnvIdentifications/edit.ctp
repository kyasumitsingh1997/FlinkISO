...
<?php echo $this->fetch('script'); ?><div id="envIdentifications_ajax">
<?php echo $this->Session->flash();?>	
<div class="nav panel panel-default">
<div class="envIdentifications form col-md-8">
<h4><?php echo __('Edit Env Identification'); ?>		
		<?php echo $this->Html->link(__('List'), array('action' => 'index'),array('id'=>'list','class'=>'label btn-info')); ?>
		
		</h4>
<?php echo $this->Form->create('EnvIdentification',array('role'=>'form','class'=>'form')); ?>
<div class="row">
			<?php
		echo "<div class='col-md-6'>".$this->Form->input('title') . '</div>'; 
		echo "<div class='col-md-6'>".$this->Form->input('env_activity_id',array('style'=>'')) . '</div>'; 
		echo "<div class='col-md-6'>".$this->Form->input('aspect_number') . '</div>'; 
		echo "<div class='col-md-12'>".$this->Form->input('aspect_details') . '</div>'; 
		echo "<div class='col-md-6'>".$this->Form->input('env_impact_id',array('style'=>'')) . '</div>'; 
...
<script>
    ...
    $().ready(function() {
        ...
        $('#EnvIdentificationEditForm').validate({        	
            rules: {
        ...
        $("#submit_id").click(function(){
            if($('#EnvIdentificationEditForm').valid()){
                 $("#submit_id").prop("disabled",true);
                 $("#submit-indicator").show();
                $('#EnvIdentificationEditForm').submit();
            }
