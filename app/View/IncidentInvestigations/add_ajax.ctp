...
<style type="text/css">
#IncidentInvestigationIncidentWitnessId_chosen{width: 100% !important}
</style>
<div id="incidentInvestigations_ajax">
<?php echo $this->Session->flash();?><div class="nav">
<div class="incidentInvestigations form col-md-8">
<h4>Add Incident Investigation</h4>
<?php echo $this->Form->create('IncidentInvestigation',array('role'=>'form','class'=>'form','default'=>false)); ?>
<div class="row">
		<fieldset>
			<?php
		...
		<div id="investigation_tab">
		  	...
		  	<div id="control_measures_currently_in_place">		    	
		    	<textarea name="data[IncidentInvestigation][control_measures_currently_in_place]" id="IncidentInvestigationControlMeasuresCurrentlyInPlace" >
		    		<h3>Add Current control measures on pleace</h3><p>Add content here</p>
		    	</textarea>
		  	</div>
		  	<div id="summery_of_findings">
		    	<textarea name="data[IncidentInvestigation][summery_of_findings]" id="IncidentInvestigationSummeryOfFindings" >
		    		<h3>Summery of findings</h3><p>Add content here</p>
		    	</textarea>
		  	</div>
		  	<div id="reason_for_incidence">
		  		<textarea name="data[IncidentInvestigation][reason_for_incidence]" id="IncidentInvestigationReasonForIncidence" >
					<h3>Reason for incidence</h3><p>Add content here</p>
		  		</textarea>
		  	</div>
...
?>
<?php echo $this->Form->submit('Submit',array('div'=>false,'class'=>'btn btn-primary btn-success','update'=>'#incidentInvestigations_ajax','async' => 'false')); ?>
...
<script>
    $.validator.setDefaults({
		...
		$().ready(function() {
			...
			$('#IncidentInvestigationInvestigationType1').change(function(){
				$('#witness').removeClass('hide').addClass('show');
				$('#personal').removeClass('show').addClass('hide');
				$('#IncidentInvestigationIncidentAffectedPersonalId').val(-1).trigger('chosen:updated');
			});
			$('#IncidentInvestigationInvestigationType0').change(function(){
				$('#personal').removeClass('hide').addClass('show');
				$('#witness').removeClass('show').addClass('hide');				
				$('#IncidentInvestigationIncidentWitnessId').val(-1).trigger('chosen:updated');				
			});
