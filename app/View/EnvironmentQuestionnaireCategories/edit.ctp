...
<h4><?php echo __('Edit Environment Questionnaire Category'); ?>		
		<?php echo $this->Html->link(__('List'), array('action' => 'index'),array('id'=>'list','class'=>'label btn-info')); ?>
		
		</h4>
<?php echo $this->Form->create('EnvironmentQuestionnaireCategory',array('role'=>'form','class'=>'form')); ?>
<div class="row">
			<?php
...
<script>
    ...
    $().ready(function() {
        $('#EnvironmentQuestionnaireCategoryEditForm').validate();
        $("#submit-indicator").hide();
        $("#submit_id").click(function(){
            if($('#EnvironmentQuestionnaireCategoryEditForm').valid()){
                 $("#submit_id").prop("disabled",true);
                 $("#submit-indicator").show();
                $('#EnvironmentQuestionnaireCategoryEditForm').submit();
            }
