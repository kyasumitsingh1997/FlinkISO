...
<h4><?php echo __('Add Evidence'); ?>		
		<?php echo $this->Html->link(__('List'), array('action' => 'index'),array('id'=>'list','class'=>'label btn-info')); ?>
		
		</h4>
<?php echo $this->Form->create('Evidence',array('role'=>'form','class'=>'form')); ?>
<div class="row">
			<?php
		echo "<div class='col-md-6'>".$this->Form->input('file_upload_id',array('style'=>'')) . '</div>'; 
		echo "<div class='col-md-12'>".$this->Form->input('description') . '</div>'; 
		echo "<div class='col-md-6'>".$this->Form->input('model_name') . '</div>'; 
		echo "<div class='col-md-6'>".$this->Form->input('record') . '</div>'; 
...
<script>
    ...
    $().ready(function() {
        $('#EvidenceAddForm').validate();
        $("#submit-indicator").hide();
        $("#submit_id").click(function(){
            if($('#EvidenceAddForm').valid()){
