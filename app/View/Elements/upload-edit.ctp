...
<?php 
    echo "<h4 class='pull-left'>" . __("Evidence Files") . "</h4><span classs='pull-right'> ". $this->Html->link('Upload new file with approval&nbsp;&nbsp;<i class="fa fa-upload" aria-hidden="true"></i>',array('controller'=>'evidences','action'=>'lists','model'=>$this->request->params['controller'],'record'=>$recordId),array('escape'=>false, 'class'=>'pull-right btn btn-success')) ."</span>"; ?>
</div>
<div class="box-body">
<table class="table table-striped table-hover table-bordered table-responsive " id="upload_table">
	<tr>
        ...
        elseif($file['FileUpload']['file_status'] == 3)echo "<tr class='warning text-warning'>";
        else echo "<tr>";
...
<?php
	echo $this->Upload->edit('upload', $usersId . '/' . $this->request->params['controller'] . '/' . $recordId,false);
	echo $this->Form->end();
?>
</div>
<?php if($this->request->controller != 'master_list_of_formats'){ ?> 
<div class="alert alert-info">Note : These are only evidence files and do not require document number, issue number, revision etc. Document versioning is done automatically via in-build version control system based on file names. System will automatically add <strong>-ver-x</strong> at the end of each file you upload. To add chage request, click on "Actions","Add Change Request".</div>
<?php } ?>
</div>
