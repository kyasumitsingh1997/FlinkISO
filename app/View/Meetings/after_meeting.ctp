...
<script>
    $().ready(function() {
        <?php 
            if($this->request->params['named']['meeting_id']){ ?>
                $('#after_meeting').load("<?php echo Router::url('/', true); ?><?php echo $this->request->params['controller'] ?>/before_meeting_view/<?php echo $this->request->params['named']['meeting_id'];?>");    
            <?php }else{ ?>
    ...
    <div class="nav panel">
        <div class="meetings form col-md-8">
            ...
            <div class="col-md-12">
                <?php 
                    if($this->request->params['named']['meeting_id']){
                        echo $this->Form->input('meeting_id', array('default'=> $this->request->params['named']['meeting_id'])); 
                    }else{
                        echo $this->Form->input('meeting_id', array('style' => 'width:100%', 'label' => __('Select Meeting'))); 
                    }?>
            </div>
