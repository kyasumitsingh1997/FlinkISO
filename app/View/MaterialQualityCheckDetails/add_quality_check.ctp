<div id="materialQualityCheckDetails_ajax" >
    ...
    <div class="nav panel panel-default">
            ...
            <?php if(!$materialQualityCheckDetail['MaterialQualityCheckDetail']['id'] || ($materialQualityCheckDetail['MaterialQualityCheckDetail']['id'] && $this->request->params['named']['approval'] == 'yes' && $materialQualityCheckDetail['MaterialQualityCheckDetail']['publish'] == false)){ ?>

            <div class="materialQualityCheckDetails form col-md-8">
            <?php echo $this->Html->script(array('jquery.validate.min','jquery-form.min','ckeditor/ckeditor','chosen.min'));?>
            <?php echo $this->fetch('script');?>
            <style type="text/css">
                .body.document-editor{padding: 2px !important}
            </style>
            <script>
                ...
                $().ready(function() {
                    $("#MaterialQualityCheckDetailCheckPerformedDate").datepicker("setDate", new Date);
                    $("#submit_id").click(function(){
                        if($('#MaterialQualityCheckDetailQuantityReceived').val() < $('#MaterialQualityCheckDetailQuantityAccepted').val()){
            ...
            <div class="row">
                <div class="col-md-2"><strong>Purchase Order</strong></div>
                <div class="col-md-10"><?php echo ':&nbsp;&nbsp;' . $deliveryChallan['PurchaseOrder']['name']; ?></div>
            </div>
            <div class="row">
            ...
            <div class="row">&nbsp;</div>
            <div class="row">
                <div class="col-md-4"><?php echo $this->Form->input('employee_id', array('default'=>$this->Session->read('User.employee_id'),'options'=>$PublishedEmployeeList)); ?></div>
                <div class="col-md-4"><?php echo $this->Form->input('check_performed_date', array('value'=>$materialQualityCheckDetail['MaterialQualityCheckDetail']['check_performed_date'])); ?></div>    
                <div class="col-md-4"><?php echo $this->Form->input('quantity_received', array('value'=> $qtyRecd, 'readonly'=>'readonly')); ?></div>                
            </div>
            <div class="row">
            ...
            <div class="row">
                <div class="col-md-12"><?php echo $this->Form->input('quantity_accepted', array('value'=>$materialQualityCheckDetail['MaterialQualityCheckDetail']['quantity_accepted'])); ?></div>
            </div>
            <div class="row">
                ...
                <?php echo $this->Form->hidden('material_id',array('value'=>$materialQualityChecks['Material']['id']));?>
            </div>
        	<?php
...
<script>
    ...
    $("[name*='date']").datepicker({
      ...
      'showTimepicker':false,
        ...
        <div class="col-md-4">
            <?php if(!$materialQualityCheckDetail['MaterialQualityCheckDetail']['id'] || ($materialQualityCheckDetail['MaterialQualityCheckDetail']['id'] && $this->request->params['named']['approval'] == 'yes' && $materialQualityCheckDetail['MaterialQualityCheckDetail']['publish'] == false)){ ?>
                <p><?php echo $this->element('approval_history',array('approvalHistorychecks'=>$approvalHistorychecks)); ?></p>
            <?php }else{ ?> 
        ...
        <div class="materialQualityCheckDetails form col-md-12">
            <div id="loadView-<?php echo $materialQualityCheckDetail['MaterialQualityCheckDetail']['id'] ;?>"></div>
                <script type="text/javascript">
                    $("#loadView-<?php echo $materialQualityCheckDetail['MaterialQualityCheckDetail']['id'] ;?>").load("<?php echo Router::url('/', true);?>/material_quality_check_details/view/<?php echo $materialQualityCheckDetail['MaterialQualityCheckDetail']['id'] ;?>");
            </script>
        </div>
        <?php } ?>
    </div>
