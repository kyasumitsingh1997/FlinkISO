    ...
    <?php foreach ($purchaseOrderDetails as $purchaseOrderDetail) { ?>
        ...
        <?php
            ...
            foreach ($received as $key => $rec) {
                ...
                foreach($rec as $r){
                    // $a = 0;
                    if($key == $purchaseOrderDetail['Product']['id'] || $key == $purchaseOrderDetail['Device']['id'] || $key == $purchaseOrderDetail['Material']['id']){
                        // $a = $a + $r;
                        $purchaseOrderDetail['PurchaseOrderDetail']['quantity'] = $purchaseOrderDetail['PurchaseOrderDetail']['quantity'] - $r;
                        echo '<tr class="text-success">';
                        echo "<td colspan='8'>Quantity Received Earlier</td>";
                        echo "<td>".$r."</td>";
...
<?php if($purchaseOrderDetail['PurchaseOrderDetail']['quantity'] == 0){ ?> 
<tr><td colspan="9"><h4  class="pull-right">Full quantity received</h4></td></tr>
<?php } else { ?> 
    ...
    <script>
        ...
        $('#delivery_challan_details<?php echo $i; ?>QuantityReceived').blur(function() {
            ...
            if(parseInt($('#delivery_challan_details<?php echo $ij; ?>Quantity').val()) < parseInt($('#delivery_challan_details<?php echo $ij; ?>QuantityReceived').val())){
                alert('NOTE : Quantity received is more that quantity ordered.');                
            }
        });
    </script>
    <?php $i++; } ?>
