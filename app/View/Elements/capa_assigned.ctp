                ...
                <?php if(isset($assigned))foreach ($assigned as $assign): ?>
                    ...
                    <td>
                        <?php if ($assign['added_in_meeting'] == 1) {
                                echo $this->Html->link('', '#', array('class' => 'fa fa-check text-succeess', 'data-placement' => 'top', 'data-toggle' => 'tooltip', 'title' => __('Added in meeting'))); ?>
                    ...
                    <td>
                        <?php
                            if ($assign['CorrectivePreventiveAction']['target_date'] > date('Y-m-d'))
                                echo $this->Html->link(__('Act'), array('controller' => 'corrective_preventive_actions', 'action' => 'edit', $assign['CorrectivePreventiveAction']['id']), array('class' => 'btn btn-xs btn-warning'));
                            else
                                echo $this->Html->link(__('Act'), array('controller' => 'corrective_preventive_actions', 'action' => 'edit', $assign['CorrectivePreventiveAction']['id']), array('class' => 'btn btn-xs btn-danger'));
                        ?>
                    </td>
<?php endforeach ?>
            </table>
