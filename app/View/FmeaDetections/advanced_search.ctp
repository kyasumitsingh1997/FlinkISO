			...
			<div class="row">
				<div class="col-md-4 pull-right">
					<div class="input-group pull-left">
						<?php echo $this->Form->create('FmeaDetection',array('role'=>'form','class'=>'no-padding no-margin','id'=>'search-form','action'=>'search')); ?>
						...
						<?php echo $this->Form->input(__('search'),array('label'=>false,'placeholder'=>'search','role'=>'form','class'=>'form-control','action'=>'search')); ?>
							...
							<div class="input-group-btn">
								...
								<ul class="dropdown-menu pull-right">
									<li class="search-dropdown"><?php echo $this->Form->input('search_field',array('multiple'=>'checkbox','options'=>array("sr_no"=>"Sr No","detection"=>"Detection","criteria"=>"Criteria","ranking"=>"Ranking","record_status"=>"Record Status","branchid"=>"Branchid","departmentid"=>"Departmentid","approved_by"=>"Approved By","prepared_by"=>"Prepared By")));?></li>		
									<li><?php echo $this->Js->submit('Submit',array('url'=>'search','div'=>false,'class'=>'btn btn-primary btn-success','update' => '#fmeaDetections_ajax','type'=>'data','method'=>'post')); ?>
;									<?php echo $this->Form->end(); ?></li>
				...
				<div class="col-md-8">
					<h4><?php echo __('Fmea Detections'); ?>
...
<?php foreach ($fmeaDetections as $fmeaDetection): ?>
	<tr>
<td width="15"><?php echo $this->Form->checkbox('rec_ids_'.$i,array('label'=>false,'value'=>$fmeaDetection['FmeaDetection']['id'],'multiple'=>'checkbox','class'=>'rec_ids')); ?></td>		<td class=" actions">
		<div class="btn-group">
		<button type="button" data-toggle="dropdown" class="dropdown-toggle btn  btn-sm btn-default "><span class=" glyphicon glyphicon-wrench"></span></button>
				...
				<li><?php echo $this->Html->link(__('View / Upload Evidance'), array('action' => 'view', $fmeaDetection['FmeaDetection']['id'])); ?></li>
				<li><?php echo $this->Html->link(__('Edit'), array('action' => 'edit', $fmeaDetection['FmeaDetection']['id'])); ?></li>
				<li><?php echo $this->Form->postLink(__('Delete'), array('action' => 'delete', $fmeaDetection['FmeaDetection']['id']),array('class'=>''), __('Are you sure you want to delete this record ?', $fmeaDetection['FmeaDetection']['id'])); ?></li>
				<li class="divider"></li>
				<li><?php echo $this->Html->link(__('Send for Approval'), '#makerchecker',array('data-toggle'=>'modal')); ?></li>
				<li><?php echo $this->Form->postLink(__('Email Record'), array('controller'=>'message','action' => 'add', $fmeaDetection['FmeaDetection']['id']),array('class'=>''), __('Are you sure ?', $fmeaDetection['FmeaDetection']['id'])); ?></li>
			</ul>
		</div>
		</td>
		...
		<td>
			<?php echo $this->Html->link($fmeaDetection['BranchIds']['name'], array('controller' => 'branches', 'action' => 'view', $fmeaDetection['BranchIds']['id'])); ?>
		</td>
		<td>
			<?php echo $this->Html->link($fmeaDetection['DepartmentIds']['name'], array('controller' => 'departments', 'action' => 'view', $fmeaDetection['DepartmentIds']['id'])); ?>
		</td>
		<td>
			<?php echo $this->Html->link($fmeaDetection['ApprovedBy']['name'], array('controller' => 'employees', 'action' => 'view', $fmeaDetection['ApprovedBy']['id'])); ?>
		</td>
		<td>
			<?php echo $this->Html->link($fmeaDetection['PreparedBy']['name'], array('controller' => 'employees', 'action' => 'view', $fmeaDetection['PreparedBy']['id'])); ?>
		</td>
		<td>
			<?php echo $this->Html->link($fmeaDetection['Company']['name'], array('controller' => 'companies', 'action' => 'view', $fmeaDetection['Company']['id'])); ?>
		</td>
	</tr>
<?php endforeach; ?>
<?php }else{ ?>
...
<?php echo $this->Js->event('click',$this->Js->request(array('action' => 'add', 'ajax'),array('async' => true, 'update' => '#fmeaDetections_ajax')));?>
...
<?php echo $this->Form->create('fmeaDetections',array('action'=>'report','target'=>'_blank','class'=>'no-padding no-margin no-background zero-height'));?>
