...
<h4><?php echo __('Add Project Activity'); ?>		
...
<div class="row">
		...
		echo "<div class='col-md-6'>".$this->Form->input('estimated_cost') . '</div>'; 
		echo "<div class='col-md-6'>".$this->Form->input('start_date') . '</div>'; 
		echo "<div class='col-md-6'>".$this->Form->input('end_date') . '</div>'; 
		echo "<div class='col-md-6'>".$this->Form->input('sequence') . '</div>'; 
...
<script>
    ...
    $().ready(function() {
        $('#ProjectActivityAddForm').validate();
        $("#submit-indicator").hide();
        $("#submit_id").click(function(){
            if($('#ProjectActivityAddForm').valid()){
                 $("#submit_id").prop("disabled",true);
                 $("#submit-indicator").show();
                $('#ProjectActivityAddForm').submit();
            }
