			...
			<div class="row">
				<div class="col-md-4 pull-right">
					<div class="input-group pull-left">
						<?php echo $this->Form->create('Document',array('role'=>'form','class'=>'no-padding no-margin','id'=>'search-form','action'=>'search')); ?>
						...
						<?php echo $this->Form->input(__('search'),array('label'=>false,'placeholder'=>'search','role'=>'form','class'=>'form-control','action'=>'search')); ?>
							...
							<div class="input-group-btn">
								...
								<ul class="dropdown-menu pull-right">
									<li class="search-dropdown"><?php echo $this->Form->input('search_field',array('multiple'=>'checkbox','options'=>array("sr_no"=>"Sr No","title"=>"Title","document_number"=>"Document Number","issue_number"=>"Issue Number","revision_number"=>"Revision Number","revision_date"=>"Revision Date","content"=>"Content","record_status"=>"Record Status","branchid"=>"Branchid","departmentid"=>"Departmentid","approved_by"=>"Approved By","prepared_by"=>"Prepared By")));?></li>		
...
<?php foreach ($documents as $document): ?>
	<tr>
		...
		<button type="button" data-toggle="dropdown" class="dropdown-toggle btn  btn-sm btn-default "><i class="fa fa-wrench" aria-hidden="true"></i></button>
				...
				<li><?php echo $this->Html->link(__('View'), array('action' => 'view', $document['Document']['id'])); ?></li>
				<li><?php echo $this->Html->link(__('Edit'), array('action' => 'edit', $document['Document']['id'])); ?></li>
				<li><?php echo $this->Form->postLink(__('Delete'), array('action' => 'delete', $document['Document']['id']),array('class'=>''), __('Are you sure you want to delete this record ?', $document['Document']['id'])); ?></li>
				<li class="divider"></li>
				...
				<li><?php echo $this->Form->postLink(__('Email Record'), array('controller'=>'message','action' => 'add', $document['Document']['id']),array('class'=>''), __('Are you sure ?', $document['Document']['id'])); ?></li>
			</ul>
		</div>
		</td>
		<td width="50"><?php echo h($document['Document']['sr_no']); ?>&nbsp;</td>
		...
		<td width="60">
			...
			<?php } ?>&nbsp;</td>
		<td><?php echo h($document['Document']['record_status']); ?>&nbsp;</td>
		<td><?php echo h($document['Document']['status_user_id']); ?>&nbsp;</td>
		<td>
			<?php echo $this->Html->link($document['BranchIds']['name'], array('controller' => 'branches', 'action' => 'view', $document['BranchIds']['id'])); ?>
		</td>
		<td>
			<?php echo $this->Html->link($document['DepartmentIds']['name'], array('controller' => 'departments', 'action' => 'view', $document['DepartmentIds']['id'])); ?>
		</td>
		<td>
			<?php echo $this->Html->link($document['ApprovedBy']['name'], array('controller' => 'employees', 'action' => 'view', $document['ApprovedBy']['id'])); ?>
		</td>
		<td>
			<?php echo $this->Html->link($document['PreparedBy']['name'], array('controller' => 'employees', 'action' => 'view', $document['PreparedBy']['id'])); ?>
		</td>
		<td>
			<?php echo $this->Html->link($document['Division']['name'], array('controller' => 'divisions', 'action' => 'view', $document['Division']['id'])); ?>
		</td>
		<td>
			<?php echo $this->Html->link($document['Company']['name'], array('controller' => 'companies', 'action' => 'view', $document['Company']['id'])); ?>
		</td>
	</tr>
<?php endforeach; ?>
<?php }else{ ?>
...
<div class="modal-body"><?php echo $this->element('advanced-search',array('postData'=>array("sr_no"=>"Sr No","title"=>"Title","document_number"=>"Document Number","issue_number"=>"Issue Number","revision_number"=>"Revision Number","revision_date"=>"Revision Date","content"=>"Content","record_status"=>"Record Status","branchid"=>"Branchid","departmentid"=>"Departmentid","approved_by"=>"Approved By","prepared_by"=>"Prepared By"),'PublishedBranchList'=>array($PublishedBranchList))); ?></div>
		<div class="modal-footer">
		<button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
		</div></div></div></div>
