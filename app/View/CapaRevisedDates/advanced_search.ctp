			...
			<div class="row">
				<div class="col-md-4 pull-right">
					<div class="input-group pull-left">
						<?php echo $this->Form->create('CapaRevisedDate',array('role'=>'form','class'=>'no-padding no-margin','id'=>'search-form','action'=>'search')); ?>
						...
						<?php echo $this->Form->input(__('search'),array('label'=>false,'placeholder'=>'search','role'=>'form','class'=>'form-control','action'=>'search')); ?>
							...
							<div class="input-group-btn">
								...
								<ul class="dropdown-menu pull-right">
									<li class="search-dropdown"><?php echo $this->Form->input('search_field',array('multiple'=>'checkbox','options'=>array("sr_no"=>"Sr No","target_date"=>"Target Date","new_revised_date_requested"=>"New Revised Date Requested","reason"=>"Reason","revised_date"=>"Revised Date","branchid"=>"Branchid","departmentid"=>"Departmentid","approved_by"=>"Approved By","prepared_by"=>"Prepared By","record_status"=>"Record Status")));?></li>		
									<li><?php echo $this->Js->submit('Submit',array('url'=>'search','div'=>false,'class'=>'btn btn-primary btn-success','update' => '#capaRevisedDates_ajax','type'=>'data','method'=>'post')); ?>
;									<?php echo $this->Form->end(); ?></li>
				...
				<div class="col-md-8">
					<h4><?php echo __('Capa Revised Dates'); ?>
...
<?php foreach ($capaRevisedDates as $capaRevisedDate): ?>
	<tr>
<td width="15"><?php echo $this->Form->checkbox('rec_ids_'.$i,array('label'=>false,'value'=>$capaRevisedDate['CapaRevisedDate']['id'],'multiple'=>'checkbox','class'=>'rec_ids')); ?></td>		<td class=" actions">
		<div class="btn-group">
		<button type="button" data-toggle="dropdown" class="dropdown-toggle btn  btn-sm btn-default "><i class="fa fa-wrench" aria-hidden="true"></i></button>
				...
				<li><?php echo $this->Html->link(__('View / Upload Evidance'), array('action' => 'view', $capaRevisedDate['CapaRevisedDate']['id'])); ?></li>
				<li><?php echo $this->Html->link(__('Edit'), array('action' => 'edit', $capaRevisedDate['CapaRevisedDate']['id'])); ?></li>
				<li><?php echo $this->Form->postLink(__('Delete'), array('action' => 'delete', $capaRevisedDate['CapaRevisedDate']['id']),array('class'=>''), __('Are you sure you want to delete this record ?', $capaRevisedDate['CapaRevisedDate']['id'])); ?></li>
				<li class="divider"></li>
				<li><?php echo $this->Html->link(__('Send for Approval'), '#makerchecker',array('data-toggle'=>'modal')); ?></li>
				<li><?php echo $this->Form->postLink(__('Email Record'), array('controller'=>'message','action' => 'add', $capaRevisedDate['CapaRevisedDate']['id']),array('class'=>''), __('Are you sure ?', $capaRevisedDate['CapaRevisedDate']['id'])); ?></li>
			</ul>
		</div>
		</td>
		...
		<td>
			<?php echo $this->Html->link($capaRevisedDate['BranchIds']['name'], array('controller' => 'branches', 'action' => 'view', $capaRevisedDate['BranchIds']['id'])); ?>
		</td>
		<td>
			<?php echo $this->Html->link($capaRevisedDate['DepartmentIds']['name'], array('controller' => 'departments', 'action' => 'view', $capaRevisedDate['DepartmentIds']['id'])); ?>
		</td>
		<td>
			<?php echo $this->Html->link($capaRevisedDate['ApprovedBy']['name'], array('controller' => 'employees', 'action' => 'view', $capaRevisedDate['ApprovedBy']['id'])); ?>
		</td>
		<td>
			<?php echo $this->Html->link($capaRevisedDate['PreparedBy']['name'], array('controller' => 'employees', 'action' => 'view', $capaRevisedDate['PreparedBy']['id'])); ?>
		</td>
		...
		<td>
			<?php echo $this->Html->link($capaRevisedDate['Company']['name'], array('controller' => 'companies', 'action' => 'view', $capaRevisedDate['Company']['id'])); ?>
		</td>
	</tr>
<?php endforeach; ?>
<?php }else{ ?>
...
<?php echo $this->Js->event('click',$this->Js->request(array('action' => 'add', 'ajax'),array('async' => true, 'update' => '#capaRevisedDates_ajax')));?>
...
<?php echo $this->Form->create('capaRevisedDates',array('action'=>'report','target'=>'_blank','class'=>'no-padding no-margin no-background zero-height'));?>
