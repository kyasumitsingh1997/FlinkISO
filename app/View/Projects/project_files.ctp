  ...
  <script type="text/javascript">
      ...
      $("#ProjectFileSearchFakeProjectFilesFormSubmit<?php echo $mid;?>").on('click',function(){
        ...
        $.ajax({
            ...
            url: "<?php echo Router::url('/', true); ?>projects/project_files/<?php echo $milestone['Milestone']['project_id'];?>/<?php echo $mid;?>",
            beforeSend: function(){
      ...
      $().ready(function() {
          ...
          $("#refresh-page-<?php echo $mid;?>").on('click',function(){
            ...
            $("#project_files_<?php echo $mid;?>").load("<?php echo Router::url('/', true); ?>/projects/project_files/<?php echo $milestone['Milestone']['project_id']?>/<?php echo $milestone['Milestone']['id']?>/<?php echo $pop ?>/<?php echo $por ;?>",function(response, status, xhr ){
              if(status == 'success'){
...
<?php if($projectFiles){ ?>
  <h4>Note : Change user functionality is only avaiable for Project Manager, Team Leaders and Project Leaders. In order to change the user, user must keep the file on HOLD.</h4>
    <?php echo $this->Form->create('ProjectFileFake','#',array('id'=>null, 'default'=>false)); ?>                    
      <table class="table table-responsive table-condensed table-bordered" id="filetable<?php echo $mid;?>">
        <thead>
          <tr>
            ...
            <th>Start Time (1st Process)</th>     
            <th>Completed Date (Last Process)</th>
            ...
            <tr class="<?php echo $catclass;?> <?php echo $sClass;?>">
                    ...
                    if($file['ProjectFile']['current_status'] == 5){
                      ...
                      if($file['ProjectFile']['file_category_id'] != -1){
                         if($file['ProjectFile']['current_status'] == 7 || $file['ProjectFile']['current_status'] == 4 || $file['ProjectFile']['current_status'] == 10 || $file['ProjectFile']['start_time'] == '' || $file['ProjectFile']['latest_start_date'] == ''){
                          if($file['ProjectFile']['last_process_id']){
              ...
              <!-- <tfoot>
                <tr class="warning">
                  ...
                  <th colspan="2">Open : <?php echo $open;?></th>
                  <th colspan="2">Closed : <?php echo $closed;?></th>
    ...
    <?php echo $this->Form->create('ProjectFile',array('controller'=>'project_files','action'=>'add_ajax','id'=>'ProjectFileAddAjaxForm'.$mid),array('id'=>'ProjectFile'.$mid, 'role'=>'form','class'=>'form','default'=>false)); ?>
      ...
      <tr>
        <td colspan="4">
          ...
          <?php echo $this->Form->input('file_names',array('label'=>'Copy-Paste file names,units,city,block in CSV format', 'type'=>'textarea','rows'=>10,'required'=>'required'));?></td>                                        
      </tr>
    </table>
    <script type="text/javascript">
