			...
			<div class="row">
				<div class="col-md-4 pull-right">
					<div class="input-group pull-left">
						<?php echo $this->Form->create('Incident',array('role'=>'form','class'=>'no-padding no-margin','id'=>'search-form','action'=>'search')); ?>
						...
						<?php echo $this->Form->input(__('search'),array('label'=>false,'placeholder'=>'search','role'=>'form','class'=>'form-control','action'=>'search')); ?>
							...
							<div class="input-group-btn">
								...
								<ul class="dropdown-menu pull-right">
									<li class="search-dropdown"><?php echo $this->Form->input('search_field',array('multiple'=>'checkbox','options'=>array("sr_no"=>"Sr No","title"=>"Title","reported_by"=>"Reported By","incident_date"=>"Incident Date","incident_reported_lag_time"=>"Incident Reported Lag Time","location"=>"Location","location_details"=>"Location Details","activity"=>"Activity","activity_details"=>"Activity Details","damage_details"=>"Damage Details","first_aid_provided"=>"First Aid Provided","first_aid_details"=>"First Aid Details","first_aid_provided_by"=>"First Aid Provided By","record_status"=>"Record Status","branchid"=>"Branchid","departmentid"=>"Departmentid","approved_by"=>"Approved By","prepared_by"=>"Prepared By")));?></li>		
									<li><?php echo $this->Js->submit('Submit',array('url'=>'search','div'=>false,'class'=>'btn btn-primary btn-success','update' => '#incidents_ajax','type'=>'data','method'=>'post')); ?>
...
<?php foreach ($incidents as $incident): ?>
	<tr class="on_page_src">
                    <td width="15"><?php echo $this->Form->checkbox('rec_ids_'.$i,array('label'=>false,'value'=>$incident['Incident']['id'],'multiple'=>'checkbox','class'=>'rec_ids')); ?></td>		<td class=" actions">
		<div class="btn-group">
		<button type="button" data-toggle="dropdown" class="dropdown-toggle btn  btn-sm btn-default "><i class="fa fa-wrench" aria-hidden="true"></i></button>
				...
				<li><?php echo $this->Html->link(__('View / Upload Evidance'), array('action' => 'view', $incident['Incident']['id'])); ?></li>
				<li><?php echo $this->Html->link(__('Edit'), array('action' => 'edit', $incident['Incident']['id'])); ?></li>
				<li><?php echo $this->Form->postLink(__('Delete'), array('action' => 'delete', $incident['Incident']['id']),array('class'=>''), __('Are you sure you want to delete this record ?', $incident['Incident']['id'])); ?></li>
				<li class="divider"></li>
				<li><?php echo $this->Html->link(__('Send for Approval'), '#makerchecker',array('data-toggle'=>'modal')); ?></li>
				<li><?php echo $this->Form->postLink(__('Email Record'), array('controller'=>'message','action' => 'add', $incident['Incident']['id']),array('class'=>''), __('Are you sure ?', $incident['Incident']['id'])); ?></li>
			</ul>
		</div>
		</td>
		...
		<td>
			<?php echo $this->Html->link($incident['BranchIds']['name'], array('controller' => 'branches', 'action' => 'view', $incident['BranchIds']['id'])); ?>
		</td>
		<td>
			<?php echo $this->Html->link($incident['DepartmentIds']['name'], array('controller' => 'departments', 'action' => 'view', $incident['DepartmentIds']['id'])); ?>
		</td>
		<td>
			<?php echo $this->Html->link($incident['ApprovedBy']['name'], array('controller' => 'employees', 'action' => 'view', $incident['ApprovedBy']['id'])); ?>
		</td>
		<td>
			<?php echo $this->Html->link($incident['PreparedBy']['name'], array('controller' => 'employees', 'action' => 'view', $incident['PreparedBy']['id'])); ?>
		</td>
		<td>
			<?php echo $this->Html->link($incident['Company']['name'], array('controller' => 'companies', 'action' => 'view', $incident['Company']['id'])); ?>
		</td>
	</tr>
<?php endforeach; ?>
<?php }else{ ?>
...
<?php echo $this->Js->event('click',$this->Js->request(array('action' => 'add', 'ajax'),array('async' => true, 'update' => '#incidents_ajax')));?>
...
<?php echo $this->Form->create('incidents',array('action'=>'report','target'=>'_blank','class'=>'no-padding no-margin no-background zero-height'));?>
