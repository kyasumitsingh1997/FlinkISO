...
<script>
    ...
    $().ready(function() {
        ...
        $('#PurchaseOrderApproveForm').validate();
        $("#submit-indicator").hide();
        $("#submit_id").click(function(){
             if($('#PurchaseOrderApproveForm').valid()){
                 $("#submit_id").prop("disabled",true);
                 $("#submit-indicator").show();
		 $("#PurchaseOrderApproveForm").submit();
             }
        });
...
<div id="purchaseOrders_ajax">
    ...
    <div class="nav panel panel-default">
        <div class="purchaseOrders form col-md-8">
            <h4><?php echo $this->element('breadcrumbs') . __('Approve Purchase Order'); ?>
          ...
          <?php echo $this->Html->link(__('Download PDF'), array('action' => 'view',$this->request->params['pass'][0].'.pdf'), array('id' => 'pdf', 'class' => 'label btn-info')); ?>
            ...
            <div class="row">
                ...
                <div class="col-md-3"><?php echo $this->Form->input('intimation'); ?></div>
...
<script>
    ...
    function choose_product_<?php echo $i; ?>(chk) {
        ...
        if (chk == 'product') {
            ...
            $('#PurchaseOrderDetail<?php echo $i; ?>MaterialId').rules('remove');
            $('#PurchaseOrderDetail<?php echo $i; ?>MaterialId').val(-1).trigger('chosen:updated');
            $('#PurchaseOrderDetail<?php echo $i; ?>Other').rules('add', {required: true
            });
