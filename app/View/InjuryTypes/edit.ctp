...
<h4><?php echo __('Edit Injury Type'); ?>		
		<?php echo $this->Html->link(__('List'), array('action' => 'index'),array('id'=>'list','class'=>'label btn-info')); ?>
		
		</h4>
<?php echo $this->Form->create('InjuryType',array('role'=>'form','class'=>'form')); ?>
<div class="row">
			<?php
...
<script>
    ...
    $().ready(function() {
        $('#InjuryTypeEditForm').validate();
        $("#submit-indicator").hide();
        $("#submit_id").click(function(){
            if($('#InjuryTypeEditForm').valid()){
                 $("#submit_id").prop("disabled",true);
                 $("#submit-indicator").show();
                $('#InjuryTypeEditForm').submit();
            }
