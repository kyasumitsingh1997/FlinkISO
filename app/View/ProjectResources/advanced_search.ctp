			...
			<div class="row">
				<div class="col-md-4 pull-right">
					<div class="input-group pull-left">
						<?php echo $this->Form->create('ProjectResource',array('role'=>'form','class'=>'no-padding no-margin','id'=>'search-form','action'=>'search')); ?>
						...
						<?php echo $this->Form->input(__('search'),array('label'=>false,'placeholder'=>'search','role'=>'form','class'=>'form-control','action'=>'search')); ?>
							...
							<div class="input-group-btn">
								...
								<ul class="dropdown-menu pull-right">
									<li class="search-dropdown"><?php echo $this->Form->input('search_field',array('multiple'=>'checkbox','options'=>array("sr_no"=>"Sr No","mandays"=>"Mandays","resource_cost"=>"Resource Cost","total_cost"=>"Total Cost","record_status"=>"Record Status","branchid"=>"Branchid","departmentid"=>"Departmentid","prepared_by"=>"Prepared By","approved_by"=>"Approved By")));?></li>		
									<li><?php echo $this->Js->submit('Submit',array('url'=>'search','div'=>false,'class'=>'btn btn-primary btn-success','update' => '#projectResources_ajax','type'=>'data','method'=>'post')); ?>
;									<?php echo $this->Form->end(); ?></li>
				...
				<div class="col-md-8">
					<h4><?php echo __('Project Resources'); ?>
...
<?php foreach ($projectResources as $projectResource): ?>
	<tr>
<td width="15"><?php echo $this->Form->checkbox('rec_ids_'.$i,array('label'=>false,'value'=>$projectResource['ProjectResource']['id'],'multiple'=>'checkbox','class'=>'rec_ids')); ?></td>		<td class=" actions">
		<div class="btn-group">
		<button type="button" data-toggle="dropdown" class="dropdown-toggle btn  btn-sm btn-default "><span class=" glyphicon glyphicon-wrench"></span></button>
				...
				<li><?php echo $this->Html->link(__('View / Upload Evidance'), array('action' => 'view', $projectResource['ProjectResource']['id'])); ?></li>
				<li><?php echo $this->Html->link(__('Edit'), array('action' => 'edit', $projectResource['ProjectResource']['id'])); ?></li>
				<li><?php echo $this->Form->postLink(__('Delete'), array('action' => 'delete', $projectResource['ProjectResource']['id']),array('class'=>''), __('Are you sure you want to delete this record ?', $projectResource['ProjectResource']['id'])); ?></li>
				<li class="divider"></li>
				<li><?php echo $this->Html->link(__('Send for Approval'), '#makerchecker',array('data-toggle'=>'modal')); ?></li>
				<li><?php echo $this->Form->postLink(__('Email Record'), array('controller'=>'message','action' => 'add', $projectResource['ProjectResource']['id']),array('class'=>''), __('Are you sure ?', $projectResource['ProjectResource']['id'])); ?></li>
			</ul>
		</div>
		</td>
		<td width="50"><?php echo h($projectResource['ProjectResource']['sr_no']); ?>&nbsp;</td>
		<td>
		...
		<td width="60">
			...
			<?php } ?>&nbsp;</td>
		<td><?php echo h($projectResource['ProjectResource']['record_status']); ?>&nbsp;</td>
		<td><?php echo h($projectResource['ProjectResource']['status_user_id']); ?>&nbsp;</td>
		<td>
			<?php echo $this->Html->link($projectResource['BranchIds']['name'], array('controller' => 'branches', 'action' => 'view', $projectResource['BranchIds']['id'])); ?>
		</td>
		<td>
			<?php echo $this->Html->link($projectResource['DepartmentIds']['name'], array('controller' => 'departments', 'action' => 'view', $projectResource['DepartmentIds']['id'])); ?>
		</td>
		<td>
			<?php echo $this->Html->link($projectResource['PreparedBy']['name'], array('controller' => 'employees', 'action' => 'view', $projectResource['PreparedBy']['id'])); ?>
		</td>
		<td>
			<?php echo $this->Html->link($projectResource['ApprovedBy']['name'], array('controller' => 'employees', 'action' => 'view', $projectResource['ApprovedBy']['id'])); ?>
		</td>
	</tr>
<?php endforeach; ?>
<?php }else{ ?>
...
<?php echo $this->Js->event('click',$this->Js->request(array('action' => 'add', 'ajax'),array('async' => true, 'update' => '#projectResources_ajax')));?>
...
<?php echo $this->Form->create('projectResources',array('action'=>'report','target'=>'_blank','class'=>'no-padding no-margin no-background zero-height'));?>
...
<div class="modal-body"><?php echo $this->element('advanced-search',array('postData'=>array("sr_no"=>"Sr No","mandays"=>"Mandays","resource_cost"=>"Resource Cost","total_cost"=>"Total Cost","record_status"=>"Record Status","branchid"=>"Branchid","departmentid"=>"Departmentid","prepared_by"=>"Prepared By","approved_by"=>"Approved By"),'PublishedBranchList'=>array($PublishedBranchList))); ?></div>
		<div class="modal-footer">
		<button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
		</div></div></div></div>
