...
setTimeout(fade_out, 10000);
    ...
    $("#project_task_submit").bind('click',function (event){
...
<div id = "project_task_ajax_1">
    <div id="project_task_ajax">
            ...
            <table class="table table-condensed checklists">
                    ...
                    <!-- <td> -->
                            <?php
                                ...
                                if ($task['TaskStatus']['task_performed'] > 0 && ($task['TaskStatus']['id'] != $editId)) {
                                    ...
                                    else echo "On going<br /><small>End Date : " . $task['Task']['end_date'] . "</small>";
                                }
                            ?>
                    <!-- </td> -->
                        <!-- <td><?php 
                    ...
                    <td><?php
                                if ($task['TaskStatus']['id']){
                                    echo $this->Js->link('<i class="fa fa-pencil-square-o" aria-hidden="true"></i>', 
                                        array('controller' => 'tasks', 'action' => 'get_project_task', $task['TaskStatus']['id']), 
                                        array('escape' => false, 'update' => '#project_task_ajax', 'async' => 'false'))
                            ?>
                    </td>
                    <td>
                            <div class="btn-group  pull-right">
                                <div id="project_taskcountdiv<?php echo $task['TaskStatus']['id'];?>" class="btn-xs btn btn-primary"></div>                                            
                                <?php echo $this->Html->link('Files','#',array('id'=>'project_taskfilesbtn'.$task['TaskStatus']['id'],'class'=>'btn btn-xs btn-info')); ?>    
                            </div>
                    
                    
                            <div id="project_taskfilesdiv<?php echo $task['TaskStatus']['id'];?>"></div>                           
                            <script>
                                
                                    $('#project_taskfilesbtn<?php echo $task["TaskStatus"]["id"];?>').click(function(){
                                        $('#project_taskfilesdiv<?php echo $task["TaskStatus"]["id"];?>').load("<?php echo Router::url('/', true); ?>tasks/project_task_ajax/<?php echo $task['TaskStatus']['id'] ?>/<?php echo $task['TaskStatus']['id']; ?>", function(response, status, xhr){});
                                });
                            </script>
                            <script>
                                    $().ready(function(){$('#project_taskcountdiv<?php echo $task['TaskStatus']['id'];?>').load('<?php echo Router::url('/', true); ?>tasks/project_task_ajax_file_count/<?php echo $task['TaskStatus']['id'] ?>', function(response, status, xhr){});});
                                    </script>
                ...
                <tr>
                    <td colspan='7'>
                            <?php
                                if ($i == 1)
                                    echo $this->Js->submit(__('Submit'), array('url' => array('controller' => 'tasks', 'action' => 'get_project_task'), 'div' => false, 'class' => 'btn btn-primary btn-success', 'update' => '#project_task_ajax_1', 'async' => 'false','id'=>'project_task_submit'));
                            ?>
                    </td>
                </tr>
            </table>
...
<?php } else{ ?>
    <div id="project_tasks_ajax" style="padding:10px">
