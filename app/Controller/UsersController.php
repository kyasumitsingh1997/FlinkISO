...
class UsersController extends AppController
{
    ...
    public function pm_dashboard($sesisonid = null){
        ...
        $openProjects = $this->Project->find('count',array('conditions'=>array('Project.current_status'=>0,'Project.soft_delete'=>0)));
        $closedProjects = $this->Project->find('count',array('conditions'=>array('Project.current_status'=>1,'Project.soft_delete'=>0)));
        $delayedProjects = $this->Project->find('count',array('conditions'=>array('Project.current_status'=>0,'Project.soft_delete'=>0,'Project.end_date <' => date('Y-m-d'))));
       
        // $projectChecklists = $this->Project->ProjectChecklist->find('list',array('conditions'=>array()));
        $this->set(compact('openProjects','closedProjects','delayedProjects'));
        ...
        $projectResourcess = $this->ProjectTimesheet->Project->ProjectResource->find('all',            
            array(
                'fields'=>array(
                  ...
                  'ProjectResource.milestone_id',
                  'ProjectResource.process_id',
                  'ProjectResource.mandays',
                  'ProjectResource.resource_cost',
                  'ProjectResource.resource_sub_total',
                  'ProjectResource.current_status',
                ),
        ...
        $this->ProjectFile->virtualFields = array(
            'closed_process'=>'select count(*) from file_processes where file_processes.project_file_id LIKE ProjectFile.id AND current_status = 5',
        ...
        $thisProject = $this->ProjectEmployee->find('first',array(
            ...
            'fields'=>array('ProjectEmployee.id','ProjectEmployee.project_id',
                'ProjectEmployee.start_date','ProjectEmployee.end_date','ProjectEmployee.project_id','ProjectEmployee.employee_id'),
            // 'order'=>array('ProjectEmployee.sr_no'=>'DESC'),
            'conditions'=>array(
                'DATE(ProjectEmployee.start_date) <= '=> date('Y-m-d'),
                'DATE(ProjectEmployee.end_date) >= '=> date('Y-m-d'),
        ...
        */
        ...
        // befor manually assigned file check, check if user is already working on any file
        // check if user is still on that project

        $this->ProjectFile->FileProcess->virtualFields = array(
            'locked_till'=>'select DISTINCT(`project_employees`.`end_date`) from `project_employees` WHERE  `project_employees.employee_id`= "'.$this->Session->read('User.employee_id').'" and  `project_employees.project_id` = "'.$thisProjectId.'" and `project_employees`.`end_date` <= "' . date('Y-m-d') . '" GROUP BY  project_employees`.`employee_id` ORDER BY `project_employees`.`start_date` DESC LIMIT 1',
        );




        $startedProcess = $this->ProjectFile->FileProcess->find('first',array(
            ...
            'conditions'=>array(
                ...
                'FileProcess.locked_till <= ' => date('Y-m-d'),
        ...
        if($startedProcess){
            $projectFile = $this->ProjectFile->find('first',array(
                'conditions'=>array('ProjectFile.id'=>$startedProcess['FileProcess']['project_file_id'])
            ));
        }
        ...
        // // first check for manually assigned file with no process so far
        ...
        if($projectFile){
            ...
            $projectFile['ProjectFile']['auto_manual'] = 0;
        ...
        if(!$projectFile){  
            ...
            $projectFile = $this->ProjectFile->find('first',array(
                ...
                'conditions'=>array(
                    ...
                    // 'FileProcess.employee_id' => $this->Session->read('User.employee_id')
                    'ProjectFile.proheck1 >'=>0,
                    'ProjectFile.closed_process'=>0,    
                    'ProjectFile.auto_manual'=>0,
                ),
                'order'=>array(
                    'ProjectFile.priority'=>'ASC',
                    'ProjectFile.sr_no'=>'ASC',
        ...
        $this->ProjectFile->hasMany['FileProcess']['limit'] = 5;
        ...
        if(!$projectFile){
            ...
            //     'ProjectFile.milestone_id',
            //     'ProjectFile.project_process_plan_id',
            //     'ProjectFile.employee_id',
            //     'ProjectFile.file_category_id',
            //     'ProjectFile.file_batch_id',
            //     'ProjectFile.single_batch',
            //     'ProjectFile.city',
            //     'ProjectFile.block',
            //     'ProjectFile.priority',
            //     'ProjectFile.assigned_date',
            //     'ProjectFile.estimated_time',
            //     'ProjectFile.completed_date',
            //     'ProjectFile.start_date',
            //     'ProjectFile.end_date',
            //     'ProjectFile.actual_time',
            //     'ProjectFile.comments',
            //     'ProjectFile.current_status',
            //     'ProjectFile.queued',
            //     'ProjectFile.checked_by',
            //     'ProjectFile.parent_id',
            //     'ProjectFile.closed_process',
            //     'ProjectFile.curr_stage',
            //     'ProjectFile.pro_pub',
            //     'ProjectFile.pro_sd',
            //     'ProjectFile.mile_pub',
            //     'ProjectFile.mile_sd',
            //     'ProjectFile.estimated_time_1',
            //     'ProjectFile.cat_on_hold',
            //     'ProjectFile.curr_employee',
            //     'ProjectFile.proheck1',
            // ),
            'conditions'=>array(
                'ProjectFile.project_id'=>$thisProjectId,
                'ProjectFile.pro_pub' =>1,
...
*/
        $projectFile = $this->ProjectFile->find('first',array(
            ...
            'order'=>array('ProjectFile.priority'=>'ASC','ProjectFile.modified'=>'DESC','ProjectProcessPlan.qc'=>'ASC')
        ));
        ...
        if(!$projectFile){
            $projectFile = $this->ProjectFile->find('first',array(
                ...
                'order'=>array('ProjectFile.priority'=>'ASC','ProjectFile.sr_no'=>'ASC','ProjectProcessPlan.qc'=>'ASC')
            ));      
        }
        ...
        if(!$projectFile){
            ...
            if($projectFile){
                ...
                $projectRe = $this->ProjectResource->find('count',array('conditions'=>array(
                    'ProjectResource.project_id'=>$projectFile['ProjectFile']['project_id'],
                    'ProjectResource.milestone_id'=>$projectFile['ProjectFile']['milestone_id'],
                    'ProjectResource.employee_id'=>$this->Session->read('User.employee_id'),
                    'ProjectResource.process_id'=>$projectFile['FileProcess'][0]['project_process_plan_id'],
                )));
                
                if($projectFile['ProjectProcessPlan']['qc'] == 1){
                ...
                if($projectRe == 0 && $dontshowskip == false){
                    $this->set('dontshow','No files are available as per your defined skill set.');
                }

                if($res_check == true){
                    // echo "Please add over all plan / detailed plans/ resources etc before adding files.";   
                    $this->set('dontshow','<b>Project : '.$projectFile['Project']['title']. '</b>.<br /><b>Milestone : '.$projectFile['Milestone']['title']. '</b>.<br /><b>File : '.$projectFile['ProjectFile']['name']. '</b>.<br /><b>Process : '.$projectFile['ProjectProcessPlan']['process']. '</b>.<br /><b>Error :</b>Please add over all plan / detailed plans/ resources etc before adding files.');
                }else if($errocheck == 0 ){
                    // echo "Please add errors before adding files.";  
                    // $this->set('dontshow','<strong>'.$projects[$thisProjectId].'</strong> : Please add errors before adding files ');
                    $this->set('dontshow','<b>Project : '.$projectFile['Project']['title']. '</b>.<br /><b>Milestone : '.$projectFile['Milestone']['title']. '</b>.<br /><b>File : '.$projectFile['ProjectFile']['name']. '</b>.<br /><b>Error :</b>Please add errors before adding files ');
        ...
        unset($this->ProjectFile->FileProcess->belongsTo['SystemTable']);
        unset($this->ProjectFile->FileProcess->belongsTo['MasterListOfFormat']);
        unset($this->ProjectFile->FileProcess->belongsTo['BranchIds']);
        unset($this->ProjectFile->FileProcess->belongsTo['DepartmentIds']);
        unset($this->ProjectFile->FileProcess->belongsTo['PreparedBy']);
        unset($this->ProjectFile->FileProcess->belongsTo['ApprovedBy']);
        unset($this->ProjectFile->FileProcess->belongsTo['CreatedBy']);
        unset($this->ProjectFile->FileProcess->belongsTo['ModifiedBy']);
        ...
        if($currentProcesses && ($currentProcesses['FileProcess']['milestone_id'] == $projectFile['ProjectFile']['milestone_id'])){
            ...
            $otherUnits = $this->Project->ProjectProcessPlan->OtherMeasurableUnit->find('list',array(
                'fields'=>array('OtherMeasurableUnit.id','OtherMeasurableUnit.unit_name'),
            'conditions'=>array(
                    'OtherMeasurableUnit.project_process_plan_id' => $currentProcesses['FileProcess']['project_process_plan_id']
                )
            ));
            $this->set('otherUnits',$otherUnits);            
        ...
        $approvalsCount = $this->_count($approvals);
        ...
        $responses = $this->ProjectQuery->ProjectQueryResponse->find('all',array(
            'recursive'=>0,
            'conditions'=>array(
        ...
        if ($this->Session->read('User.is_mr') == true) {
            ...
            $blockedUserCount = $this->_count($blockedUser);
    ...
    */
            ...
            if (!empty($this->request->data)) {
                ...
                if ($this->_count($old_pwd)) {
                    if (!in_array($user['User']['password'], $old_pwd)) {
                        ...
                        if ($this->_count($old_pwd) > $pwd_repeat_cnt) {
    ...
    public function personal_admin(){
        ...
        return $this->_count($houseKeepings);
    }
    ...
    public function appraisal_answers($token = null)
    {
        ...
        if ($this->_count($appraisal)) {
            ...
            if ($this->_count($employeeAppraisals) > 0) {
                if ($this->request->is('post') || $this->request->is('put')) {
