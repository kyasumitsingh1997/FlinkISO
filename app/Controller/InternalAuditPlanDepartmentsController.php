...
class InternalAuditPlanDepartmentsController extends AppController {
    ...
    public function advanced_search() {
        ...
        if ($this->request->query['from-date']) {
            $conditions[] = array('InternalAuditPlanDepartment.created >' => date('Y-m-d h:i:s', strtotime($this->request->query['from-date'])), 'InternalAuditPlanDepartment.created <' => date('Y-m-d h:i:s', strtotime($this->request->query['to-date'])));
        }
    ...
    public function plan_add_ajax($plan_id) {
        ...
        if ($this->request->is('post')) {
            unset($this->request->data['InternalAuditPlanDepartment']['endTime']);
            $dateRange = split('-', $this->request->data['InternalAuditPlanDepartment']['startTime']);
            $start_date = rtrim(ltrim($dateRange[0]));
            $end_date = rtrim(ltrim($dateRange[1]));
            
            $this->request->data['InternalAuditPlanDepartment']['start_time'] = date('Y-m-d',strtotime($start_date));
            $this->request->data['InternalAuditPlanDepartment']['end_time'] = date('Y-m-d',strtotime($end_date));
    ...
    public function edit($id = null) {
        ...
        if ($this->request->is('post') || $this->request->is('put')) {
            ...
            unset($this->request->data['InternalAuditPlanDepartment']['end_time']);
            $dateRange = split('-', $this->request->data['InternalAuditPlanDepartment']['start_time']);
            $start_date = rtrim(ltrim($dateRange[0]));
            $end_date = rtrim(ltrim($dateRange[1]));
        ...
        $employees = $this->InternalAuditPlanDepartment->Employee->find('list', array('conditions' => array('Employee.publish' => 1, 'Employee.soft_delete' => 0)));
        $listOfTrainedInternalAuditors = $this->InternalAuditPlanDepartment->ListOfTrainedInternalAuditor->find('list', array('fields' => array('ListOfTrainedInternalAuditor.id', 'ListOfTrainedInternalAuditor.name'), 'conditions' => array('ListOfTrainedInternalAuditor.publish' => 1, 'ListOfTrainedInternalAuditor.soft_delete' => 0), 'recursive' => 0));
        $systemTables = $this->InternalAuditPlanDepartment->SystemTable->find('list', array('conditions' => array('SystemTable.publish' => 1, 'SystemTable.soft_delete' => 0)));
        $masterListOfFormats = $this->InternalAuditPlanDepartment->MasterListOfFormat->find('list', array('conditions' => array('MasterListOfFormat.publish' => 1, 'MasterListOfFormat.soft_delete' => 0)));
        
        $processes = $this->InternalAuditPlanDepartment->InternalAuditPlan->Process->find('list', array('conditions' => array('Process.publish' => 1, 'Process.soft_delete' => 0)));
        $riskAssessments = $this->InternalAuditPlanDepartment->InternalAuditPlan->RiskAssessment->find('list', array('conditions' => array('RiskAssessment.publish' => 1, 'RiskAssessment.soft_delete' => 0)));

        $this->set(compact('internalAuditPlans', 'departments', 'branches', 'employees', 'listOfTrainedInternalAuditors', 'systemTables', 'masterListOfFormats','processes','riskAssessments'));
        $count = $this->InternalAuditPlanDepartment->find('count');
        $published = $this->InternalAuditPlanDepartment->find('count', array('conditions' => array('InternalAuditPlanDepartment.publish' => 1)));
        $unpublished = $this->InternalAuditPlanDepartment->find('count', array('conditions' => array('InternalAuditPlanDepartment.publish' => 0)));
