...
<h4><?php echo __('Edit Process Team'); ?>		
		<?php echo $this->Html->link(__('List'), array('action' => 'index'),array('id'=>'list','class'=>'label btn-info')); ?>
		
		</h4>
<?php echo $this->Form->create('ProcessTeam',array('role'=>'form','class'=>'form')); ?>
<div class="row">
			<?php
		...
		echo "<div class='col-md-6'>".$this->Form->input('process_type') . '</div>'; 
		...
		echo "<div class='col-md-12'>".$this->Form->input('target') . '</div>'; 
		echo "<div class='col-md-12'>".$this->Form->input('measurement_details') . '</div>'; 
...
<script>
    ...
    $().ready(function() {
        $('#ProcessTeamEditForm').validate();
        $("#submit-indicator").hide();
        $("#submit_id").click(function(){
            if($('#ProcessTeamEditForm').valid()){
                 $("#submit_id").prop("disabled",true);
                 $("#submit-indicator").show();
                $('#ProcessTeamEditForm').submit();
            }
