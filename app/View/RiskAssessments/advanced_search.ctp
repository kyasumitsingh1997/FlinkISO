...
<?php foreach ($riskAssessments as $riskAssessment): ?>
	<tr>
<td width="15"><?php echo $this->Form->checkbox('rec_ids_'.$i,array('label'=>false,'value'=>$riskAssessment['RiskAssessment']['id'],'multiple'=>'checkbox','class'=>'rec_ids')); ?></td>		<td class=" actions">
		<div class="btn-group">
		<button type="button" data-toggle="dropdown" class="dropdown-toggle btn  btn-sm btn-default "><i class="fa fa-wrench" aria-hidden="true"></i></button>
				...
				<li><?php echo $this->Html->link(__('View / Upload Evidance'), array('action' => 'view', $riskAssessment['RiskAssessment']['id'])); ?></li>
...
<?php echo $this->Form->create('riskAssessments',array('action'=>'report','target'=>'_blank','class'=>'no-padding no-margin no-background zero-height'));?>
