...
class MaterialQualityChecksController extends AppController {
    ...
    public function _get_system_table_id() {
      ...
      public function afterSave($created) {
        $id = $this->MaterialQualityCheck->getInsertID(); 
        $this->loadModel('Material');
        $MaterialQualityChecks = $this->MaterialQualityCheck->find('first', array('conditions' => array('MaterialQualityCheck.id' => $id,'MaterialQualityCheck.publish' => 1, 'MaterialQualityCheck.soft_delete' => 0), 'recursive' => -1));
        if($MaterialQualityChecks['MaterialQualityCheck']['material_id']){
            $status = 1;
        }else{
            $status = 0;
        }
        $Materials = $this->Material->find('first', array('conditions' => array('Material.id' => $MaterialQualityChecks['MaterialQualityCheck']['material_id']), 
            'recursive' => -1));
        
        if($Materials['Material']['id']){
            $data['id'] = $Materials['Material']['id'];
            $data['material_qc_status'] = $status;
            $this->Material->save($data, false);
        }
        ...
        if ($this->request->is('post')) {
            $flag = 0;
                ...
                foreach ($this->request->data['MaterialQC'] as $val) {
                    ...
                    $data['MaterialQualityCheck']['qc_template'] = $val['qc_template'];
            ...
            if($this->MaterialQualityCheck->id){
                ...
                // $this->_update_stocks($this->request->data['MaterialQualityCheck']['material_id'],null);
            }
            if ($this->MaterialQualityCheck->save($this->request->data)) {
                ...
                if ($this->_show_evidence() == true)
                    $this->redirect(array('controller'=>'materials', 'action' => 'view',$this->request->data['MaterialQualityCheck']['id']));
                else
                    $this->redirect(array('controller'=>'materials', 'action' => 'view',$this->request->data['MaterialQualityCheck']['id']));
            } else {
    ...
    public function delete($id = null) {
        ...
        if (!empty($id)) {
            ...
            foreach ($MaterialQualityChecks as $MaterialQualityCheck) {
             ...
             if(isset( $MaterialQualityCheck['MaterialQualityCheck'])){
                $MaterialQualityCheck['MaterialQualityCheck']['model_action'] = $this->params['action'];
                $MaterialQualityCheck['MaterialQualityCheck']['system_table_id'] = $this->_get_system_table_id();
                $modelName = $this->modelClass;
                $this->$modelName->save( $MaterialQualityCheck['MaterialQualityCheck'], false);
            }
        }
        if ($flag) {
            $this->Session->setFlash(__('Material quality check deleted'));
    ...
    public function quality_check($deliveryChallanId = null, $materialId = null, $class = null, $deliveryChallanDetailId = null) {
        // $deliveryChallanId = $this->request->params['pass'][0];
        // $materialId = $this->request->params['pass'][1];
        // $deliveryChallanDetailId = $this->request->params['pass'][2];
        ...
        $flag = 0;
        $materialQualityChecks = $this->MaterialQualityCheck->find('all', array(
            'conditions' => array(
                'MaterialQualityCheck.publish' => 1, 'MaterialQualityCheck.soft_delete' => 0, 
                'MaterialQualityCheck.material_id' => $materialId), 'recursive' => -1, 'order' => 'sr_no'));
        
        
        $this->loadModel('MaterialQualityCheckDetail');
        foreach ($materialQualityChecks as $key => $materialQualityCheck) {
            $materialQualityCheckDetails = $this->MaterialQualityCheckDetail->find('first', array(
                'conditions' => array('MaterialQualityCheckDetail.publish' => 1, 
                    'MaterialQualityCheckDetail.material_quality_check_id' => $materialQualityCheck['MaterialQualityCheck']['id'], 
                    'MaterialQualityCheckDetail.soft_delete' => 0, 
                    'MaterialQualityCheckDetail.delivery_challan_detail_id' => $deliveryChallanDetailId), 'recursive' => -1));
        ...
        $this->set(compact('materialQualityChecks', 'deliveryChallanId', 'materialId','deliveryChallanDetailId'));
    }
