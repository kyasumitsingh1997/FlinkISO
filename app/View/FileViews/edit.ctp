...
<h4><?php echo __('Edit File View'); ?>		
		<?php echo $this->Html->link(__('List'), array('action' => 'index'),array('id'=>'list','class'=>'label btn-info')); ?>
		
		</h4>
<?php echo $this->Form->create('FileView',array('role'=>'form','class'=>'form')); ?>
<div class="row">
			<?php
...
<script>
    ...
    $().ready(function() {
        $('#FileViewEditForm').validate();
        $("#submit-indicator").hide();
        $("#submit_id").click(function(){
            if($('#FileViewEditForm').valid()){
                 $("#submit_id").prop("disabled",true);
                 $("#submit-indicator").show();
                $('#FileViewEditForm').submit();
            }
