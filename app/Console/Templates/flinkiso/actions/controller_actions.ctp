 ...
 */
	public function <?php echo $admin ?>advanced_search() {
		...
		$conditions = array();
			if($this->request->query['keywords']){
				...
				foreach($search_keys as $search_key):
					...
					if($this->request->query['strict_search'] == 0)$search_array[] = array('<?php echo $currentModelName ?>.'.$search => $search_key);
		...
		if($this->request->query['from-date']){
			$conditions[] = array('<?php echo $currentModelName ?>.created >'=>date('Y-m-d h:i:s',strtotime($this->request->query['from-date'])),'<?php echo $currentModelName ?>.created <'=>date('Y-m-d h:i:s',strtotime($this->request->query['to-date'])));
		}
 ...
 */
	public function <?php echo $admin ?>add_ajax() {
		...
		if ($this->request->is('post')) {
			$this->request->data['<?php echo $currentModelName; ?>']['system_table_id'] = $this->_get_system_table_id();
 ...
 * @return void
 */
	public function <?php echo $admin; ?>approve($id = null, $approvalId = null) {
		if (!$this-><?php echo $currentModelName; ?>->exists($id)) {
			throw new NotFoundException(__('Invalid <?php echo strtolower($singularHumanName); ?>'));
		}
 ...
 * @return void
 */
	public function <?php echo $admin; ?>delete($id = null, $parent_id = NULL) {
