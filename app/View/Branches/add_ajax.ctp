...
<script>
    ...
    $("#submit-indicator").hide();
        ...
        $('#BranchName').blur(function() {
            $("#getBranch").load('<?php echo Router::url('/', true); ?><?php echo $this->request->params['controller'] ?>/get_branch_name/' + btoa(this.value), function(response, status, xhr) {
                if (response != "") {
                    $('#BranchName').val('');
    ...
    <div class="nav">
        <div class="branches form col-md-8">
            ...
            <div class="row">
                ...
                <div class="col-md-12"><?php echo $this->Form->input('details', array('placeholder'=>'Contact person/ address / phone / anyother information can be added here.', 'label' => __('Branch Details/Activities'))); ?></div>
                <div class="col-md-12"><?php echo $this->Form->input('departments', array('options'=>$PublishedDepartmentList, 'label' => __('Departments under this branch/location'),'name'=>'data[Branch][departments][]', 'multiple')); ?></div>
                <?php
