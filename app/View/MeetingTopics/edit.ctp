...
<h4><?php echo __('Edit Meeting Topic'); ?>		
		<?php echo $this->Html->link(__('Back'), array('controller'=>'meetings', 'action' => 'view',$this->data['MeetingTopic']['meeting_id']),array('id'=>'','class'=>'label btn-info')); ?>
...
<div class="row">
		...
		echo "<div class='col-md-12'>".$this->Form->input('topic_text',array('disabled')) . '</div>'; 
		...
		echo "<div class='col-md-6'>".$this->Form->input('action_taken_date',array('default'=>date('Y-m-d'))) . '</div>'; 
...
<script>
    ...
    $().ready(function() {
        ...
        $('#MeetingTopicEditForm').validate();
        $("#submit-indicator").hide();
        $("#submit_id").click(function(){
            if($('#MeetingTopicEditForm').valid()){
                 $("#submit_id").prop("disabled",true);
                 $("#submit-indicator").show();
                $('#MeetingTopicEditForm').submit();
            }
