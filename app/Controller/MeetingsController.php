...
class MeetingsController extends AppController {
    ...
    public function index() {
        ...
        $this->paginate = array('limit'=>30, 'order' => array('Meeting.sr_no' => 'DESC'), 'conditions' => array($conditions,'Meeting.meeting_type'=>0),
            'joins' => array(array(
      ...
      public function external_index() {
        ...
        $conditions = $this->_check_request();
        $this->paginate = array('limit'=>30, 'order' => array('Meeting.sr_no' => 'DESC'), 'conditions' => array($conditions,'Meeting.meeting_type'=>1),
            // 'joins' => array(array(
            //         'table' => 'meeting_employees',
            //         'alias' => 'meeting_employees',
            //         'type' => 'inner',
    ...
    public function meeting_view($id = null) {
        ...
        $meetingStatuses = $this->Meeting->customArray['meeting_status'];
        $this->set(compact('meetingStatuses'));
    ...
    public function view($id = null) {
        ...
        $abs = array_diff($emps,$atts);
        $meeting['Meeting']['Absent']  = implode(', ', $abs);
        ...
        $standards = $this->Meeting->Standard->find('list');
    ...
    public function before_meeting_view($id = null) {
        ...
        $projectActivities = $this->ProjectActivity->find('list',array('conditions'=>array('ProjectActivity.publish'=>1,'ProjectActivity.soft_delete'=>0)));
        ...
        $meeting_statuses = $this->Meeting->customArray['meeting_status'];
        $this->set('meeting_statuses',$meeting_statuses);
    ...
    public function approve($id = null, $approvalId = null) {
        ...
        if ($this->request->is('post') || $this->request->is('put')) {
            if($this->request->data['Meeting']['meeting_status'] == 2){
                $getTopics = $this->Meeting->MeetingTopic->find('count',array('MeetingTopic.meeting_id'=>$this->request->data['Meeting']['id'],'MeetingTopic.action_status'=>0));
                if($getTopics != 0){
                    $this->Session->setFlash(__('Meeting can not be closed. Some topics are open.'));
            ...
            $this->request->data['Meeting']['standard_id'] = json_encode($this->request->data['Meeting']['standard_id']);
            if ($this->Meeting->save($this->request->data, false)) {
                ...
                if(isset($this->request->data['AdditionalTopics']['ProjectActivity'])){
                    foreach ($this->request->data['AdditionalTopics']['ProjectActivity'] as $val) {
                        if (isset($val['project_activity_id']) && $val['project_activity_id'] != 0) {
                            $this->MeetingTopic->create();
                            $val['meeting_id'] = $this->Meeting->id;
                            $val['publish'] = $this->request->data['Meeting']['publish'];
                            ...
                            $val['system_table_id'] = $this->_get_system_table_id();
                            $this->loadModel('ProjectActivity');
                            $projectActivityDetails = $this->ProjectActivity->find('first', array('conditions' => array('ProjectActivity.publish' => 1, 'ProjectActivity.soft_delete' => 0, 'ProjectActivity.id' => $val['project_activity_id']), 'recursive' => -1));
                            $val['title'] = "Project Activity";
                            $val['current_status'] = $projectActivityDetails['ProjectActivity']['current_status'];
                    ...
                    if (isset($val['change_addition_deletion_request_id']) && $val['change_addition_deletion_request_id'] != 0) {
                        ...
                        if(count($docRequestDetails)){
                            ...
                            if(isset($masterListOfFormats['MasterListOfFormat']['title'])){
                                ...
                                $fileUpload = $this->FileUpload->find('first',array(
                                    'recursive' => '-1',
                                    'fields' => array('FileUpload.file_details','FileUpload.id','FileUpload.file_type'),
                                    'conditions'=>array('FileUpload.id'=>$docRequestDetails['ChangeAdditionDeletionRequest']['file_upload_id'])));
                                $val['title'] = $fileUpload['FileUpload']['file_details'].'.'.$fileUpload['FileUpload']['file_type'];
                            }
                ...
                $employees = $this->Employee->find('all',array(
                    'recursive'=>'-1',
                    'fields'=>array('Employee.id','Employee.personal_email','Employee.office_email'),
                    'conditions'=>array('Employee.id'=>$this->request->data['MeetingEmployee_employee_id'])));
                
                foreach ($employees as $employee) {
                    if($employee['Employee']['office_email']) $email[] = $employee['Employee']['office_email'];
                    elseif($employee['Employee']['office_email'] == NULL && $employee['Employee']['personal_email'] != NULL) $email[] =  $employee['Employee']['personal_email'];
                    
                }   


                if($this->request->data['Meeting']['send_email'] && $this->request->data['Meeting']['publish']){
                    if(Configure::read('evnt') == 'Dev')$env = 'DEV';
                    elseif(Configure::read('evnt') == 'Qa')$env = 'QA';
                    else $env = "";

                        $this->loadModel('Company');
                        $smtpSetup = $this->Company->find('first', array('conditions'=> array('Company.id'=>$this->Session->read('User.company_id')), 'fields'=> array('smtp_setup','is_smtp'),'recursive'=>-1));
                        ...
                        $EmailConfig->subject('FlinkISO: Meeting is scheduled on : ' . $this->request->data['Meeting']['scheduled_meeting_from']);
                        $baseurl = Router::url('/', true) . $this->request->params['controller'] . "/reset_password/" . $this->User->data['User']['password_token'];
                        $EmailConfig->template('meetingschedule');
                        $EmailConfig->viewVars(array('baseurl' => $baseurl,'env' => $env, 'app_url' => FULL_BASE_URL));
        ...
        foreach ($meetingTopic as $key => $val) {
            ...
            if ($val['MeetingTopic']['project_activity_id'] != NULL) {
                $additionalTopics['ProjectActivity'][] = $val['MeetingTopic']['project_activity_id'];
            }else {
        ...
        $allProjectActivities = $this->ProjectActivity->find('all',array('conditions'=>array('ProjectActivity.publish'=>1,'ProjectActivity.soft_delete'=>0)));
        $meeting_statuses = $this->Meeting->customArray['meeting_status'];
        $standards = $this->Meeting->Standard->find('list',array('conditions'=>array('Standard.publish'=>1,'Standard.soft_delete'=>0)));
        $this->set(compact('allChangeAdditionDeletionRequests', 'allCorrectivePreventiveActions', 'allCustomerFeedbacks', 'allCustomerComplaints', 'allInternalAuditPlans', 'allSummeryOfSupplierEvaluations', 'additionalTopics','allProjectActivities','meeting_statuses','standards'));
        
        $meetings = $this->Meeting->find('list', array('fields'=>array('Meeting.actual_meeting_from','Meeting.actual_meeting_from'), 'conditions' => array('Meeting.publish' => 1, 'Meeting.soft_delete' => 0)));
        foreach ($meetings as $key => $value) {
            $newMeetings[date('Y-m-d',strtotime($value))] = date('Y-m-d',strtotime($value));
        }
        $meetings = $newMeetings;
        $standards = $this->Meeting->Standard->find('list',array('conditions'=>array('Standard.publish'=>1,'Standard.soft_delete'=>0)));
        
        $this->set(compact('systemTables', 'masterListOfFormats','standards','meetings'));
    }
    ...
    public function edit($id = null) {
        ...
        $standards = $this->Meeting->Standard->find('list',array('conditions'=>array('Standard.publish'=>1,'Standard.soft_delete'=>0)));
        // $this->set(compact('allChangeAdditionDeletionRequests', 'allCorrectivePreventiveActions', 'allCustomerFeedbacks', 'allCustomerComplaints', 'allInternalAuditPlans', 'allSummeryOfSupplierEvaluations', 'additionalTopics','allProjectActivities','meeting_statuses','standards'));
        $customers = $this->Meeting->Customer->find('list',array('conditions'=>array('Customer.publish'=>1,'Customer.soft_delete'=>0)));
        $supplierRegistrations = $this->Meeting->SupplierRegistration->find('list',array('conditions'=>array('SupplierRegistration.publish'=>1,'SupplierRegistration.soft_delete'=>0)));
        $meetingType = $this->Meeting->customArray['meeting_type'];
        
        $this->set(compact('allChangeAdditionDeletionRequests', 'allCorrectivePreventiveActions', 'allCustomerFeedbacks', 'allCustomerComplaints', 'allInternalAuditPlans', 'allSummeryOfSupplierEvaluations','allProjectActivities','meetingType','customers','supplierRegistrations'));
    ...
    public function after_meeting() {
        ...
        // $meetings = $this->Meeting->find('list', array('conditions' => array('Meeting.publish' => 1, 'Meeting.soft_delete' => 0,'Meeting.meeting_status'=>3)));
        $meetings = $this->Meeting->find('list', array('conditions' => array('Meeting.publish' => 1, 'Meeting.soft_delete' => 0)));
        $this->set('meetings', $meetings);
        ...
        if ($this->request->is('post')) {
            ...
            $data['meeting_status'] = $this->request->data['AfterMeeting']['meeting_status'];
    ...
    public function add_ajax() {
        ...
        if ($this->request->is('post')) {
            $this->request->data['Meeting']['meeting_status'] = 0;
            $this->request->data['Meeting']['standard_id'] = json_encode($this->request->data['Meeting']['standard_id']);
            $this->request->data['Meeting']['system_table_id'] = $this->_get_system_table_id();
