...
<h4><?php echo __('Add Project Resource'); ?>		
...
<script>
    $.validator.setDefaults({
    	...
    	errorPlacement: function(error, element) {
			...
			$(element).attr('name') == 'data[ProjectResource][project_activity_id]')
						{	
    ...
    $().ready(function() {
        ...
        $('#ProjectResourceAddForm').validate({        	
            rules: {
				...
				"data[ProjectResource][project_activity_id]": {
                		greaterThanZero: true,
					},
        ...
        $("#submit_id").click(function(){
            if($('#ProjectResourceAddForm').valid()){
                 $("#submit_id").prop("disabled",true);
                 $("#submit-indicator").show();
                $('#ProjectResourceAddForm').submit();
            }
		...
		$('#ProjectResourceProjectActivityId').change(function() {
			if ($(this).val() != -1 && $(this).next().next('label').hasClass("error")) {
				$(this).next().next('label').remove();
			}
		});	
