			...
			<div class="row">
				<div class="col-md-4 pull-right">
					<div class="input-group pull-left">
						<?php echo $this->Form->create('FileError',array('role'=>'form','class'=>'no-padding no-margin','id'=>'search-form','action'=>'search')); ?>
						...
						<?php echo $this->Form->input(__('search'),array('label'=>false,'placeholder'=>'search','role'=>'form','class'=>'form-control','action'=>'search')); ?>
							...
							<div class="input-group-btn">
								...
								<ul class="dropdown-menu pull-right">
									<li class="search-dropdown"><?php echo $this->Form->input('search_field',array('multiple'=>'checkbox','options'=>array("sr_no"=>"Sr No","name"=>"Name","total_units"=>"Total Units","total_errors"=>"Total Errors","record_status"=>"Record Status","branchid"=>"Branchid","departmentid"=>"Departmentid","prepared_by"=>"Prepared By","approved_by"=>"Approved By")));?></li>		
									<li><?php echo $this->Js->submit('Submit',array('url'=>'search','div'=>false,'class'=>'btn btn-primary btn-success','update' => '#fileErrors_ajax','type'=>'data','method'=>'post')); ?>
...
<?php foreach ($fileErrors as $fileError): ?>
	<tr>
<td width="15"><?php echo $this->Form->checkbox('rec_ids_'.$i,array('label'=>false,'value'=>$fileError['FileError']['id'],'multiple'=>'checkbox','class'=>'rec_ids')); ?></td>		<td class=" actions">
		<div class="btn-group">
		<button type="button" data-toggle="dropdown" class="dropdown-toggle btn  btn-sm btn-default "><span class=" glyphicon glyphicon-wrench"></span></button>
				...
				<li><?php echo $this->Html->link(__('View / Upload Evidance'), array('action' => 'view', $fileError['FileError']['id'])); ?></li>
				<li><?php echo $this->Html->link(__('Edit'), array('action' => 'edit', $fileError['FileError']['id'])); ?></li>
				<li><?php echo $this->Form->postLink(__('Delete'), array('action' => 'delete', $fileError['FileError']['id']),array('class'=>''), __('Are you sure you want to delete this record ?', $fileError['FileError']['id'])); ?></li>
				<li class="divider"></li>
				<li><?php echo $this->Html->link(__('Send for Approval'), '#makerchecker',array('data-toggle'=>'modal')); ?></li>
				<li><?php echo $this->Form->postLink(__('Email Record'), array('controller'=>'message','action' => 'add', $fileError['FileError']['id']),array('class'=>''), __('Are you sure ?', $fileError['FileError']['id'])); ?></li>
			</ul>
		</div>
		</td>
		<td width="50"><?php echo h($fileError['FileError']['sr_no']); ?>&nbsp;</td>
		<td><?php echo h($fileError['FileError']['name']); ?>&nbsp;</td>
		<td>
		...
		<td width="60">
			...
			<?php } ?>&nbsp;</td>
		<td><?php echo h($fileError['FileError']['record_status']); ?>&nbsp;</td>
		<td><?php echo h($fileError['FileError']['status_user_id']); ?>&nbsp;</td>
		<td>
			<?php echo $this->Html->link($fileError['BranchIds']['name'], array('controller' => 'branches', 'action' => 'view', $fileError['BranchIds']['id'])); ?>
		</td>
		<td>
			<?php echo $this->Html->link($fileError['DepartmentIds']['name'], array('controller' => 'departments', 'action' => 'view', $fileError['DepartmentIds']['id'])); ?>
		</td>
		<td>
			<?php echo $this->Html->link($fileError['PreparedBy']['name'], array('controller' => 'employees', 'action' => 'view', $fileError['PreparedBy']['id'])); ?>
		</td>
		<td>
			<?php echo $this->Html->link($fileError['ApprovedBy']['name'], array('controller' => 'employees', 'action' => 'view', $fileError['ApprovedBy']['id'])); ?>
		</td>
	</tr>
<?php endforeach; ?>
<?php }else{ ?>
...
<?php echo $this->Js->event('click',$this->Js->request(array('action' => 'add', 'ajax'),array('async' => true, 'update' => '#fileErrors_ajax')));?>
...
<?php echo $this->Form->create('fileErrors',array('action'=>'report','target'=>'_blank','class'=>'no-padding no-margin no-background zero-height'));?>
