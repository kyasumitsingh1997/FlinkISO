...
<h4><?php echo __('Edit Hazard Type'); ?>		
		<?php echo $this->Html->link(__('List'), array('action' => 'index'),array('id'=>'list','class'=>'label btn-info')); ?>
		
		</h4>
<?php echo $this->Form->create('HazardType',array('role'=>'form','class'=>'form')); ?>
<div class="row">
			<?php
...
<script>
    ...
    $().ready(function() {
        $('#HazardTypeEditForm').validate();
        $("#submit-indicator").hide();
        $("#submit_id").click(function(){
            if($('#HazardTypeEditForm').valid()){
                 $("#submit_id").prop("disabled",true);
                 $("#submit-indicator").show();
                $('#HazardTypeEditForm').submit();
            }
