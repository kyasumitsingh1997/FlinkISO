					...
					foreach($benchMarks as $benchMark): ?>
						<div class="row">
							<div class="col-md-12">
								<div class="panel panel-default">
									<div class="panel-heading">
										...
										<div class="row">
											<div class="col-md-12">
												<style type="text/css">
													#main-slider-<?php echo $benchMark['branch']['id']; ?>{background:#ccc; margin:10px 0; }
													#main-slider-<?php echo $benchMark['branch']['id']; ?> .ui-slider-range{background: green;
										}
								</style>
								<script>

										$(function() {
											$( "#main-slider-<?php echo $benchMark['branch']['id']; ?>" ).slider({
											range: "min",
											min: 0,
											max: 500,
											value: 0,
											slide: function( event, ui ) {
												$( "#BenchMarkForm-<?php echo $i ?> .sld" ).html(ui.value);
												$( "#BenchMarkForm-<?php echo $i ?> .sld-a" ).slider("value",ui.value);
												$( "#main-slider-<?php echo $benchMark['branch']['id']; ?>" ).val(ui.value);
												...
												$( "#BenchMarkForm-<?php echo $i ?> input[name*='val']" ).val(ui.value);
											},
								...
								<div class="col-md-12 progress-bar-success" id="main-slider-<?php echo $benchMark['branch']['id']; ?>"></div>
									...
									<script>
				 					...
				 					<h6 class="col-md-10 no-padding">
										<span class="pull-left"><?php echo $value['name']; ?> / per day </span>&nbsp;&nbsp;
											<div class="text-success pull-left" id="notice-<?php echo $benchMark['branch']['id']; ?>-
												<?php echo $value['id']; ?>"></div>
									</h6>
								 	<div id="vals-<?php echo $benchMark['branch']['id']; ?>-<?php echo $value['id']; ?>" class="col-md-2 pull-right sld">
								 		<?php echo $value['benchmark']; ?>
								 	</div>
									<div class="col-md-12 sld-a" id="slider-<?php echo $benchMark['branch']['id']; ?>-<?php echo $value['id']; ?>"></div>
										<?php echo $this->Form->hidden('department.'.$value['id'].'.id',array('value'=>$value['id'])); ?>
										
										<?php echo $this->Form->hidden('benchmark.'.$value['id'].'.val',
											array(
												'class'=>' cng',
												'id'=>$benchMark['branch']['id'].'-'.$value['id'],
												'value'=>$value['benchmark'])); 
										?>
										
										<?php if(isset($value['benchmark_id'])){
											echo $this->Form->hidden('benchmark.'.$value['id'].'.id',
												array(
													'class'=>' cng',
													'id'=>$benchMark['branch']['id'].'-'.$value['id'],
													'value'=>$value['benchmark_id'])); 
											}
										?>
								...
								<?php echo $this->Form->hidden('state.id', array('value'=>$this->Session->read('User.state_id'))); ?>
								<?php
