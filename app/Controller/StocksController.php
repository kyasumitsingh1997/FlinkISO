...
class StocksController extends AppController {
    ...
    public function advanced_search() {
        if ($this->request->is('post')) {
            ...
            if ($this->request->query['from-date']) {
                $conditions[] = array('Stock.created >' => date('Y-m-d h:i:s', strtotime($this->request->query['from-date'])), 'Stock.created <' => date('Y-m-d h:i:s', strtotime($this->request->query['to-date'])));
            }
            ...
            if (isset($this->request->data['material_id'])) {
                ...
                foreach ($selectedMaterial as $key => $value):
                    ...
                    while (strtotime($startDate) <= strtotime($endDate)) {
                        ...
                        $outStock = $this->Stock->find('all', array(
                            ...
                            'fields' => array('SUM(Stock.quantity_consumed) as qtyconsumed'),
                            'recursive' => 0
                        ));
                        // debug($outStock);
                        
                        $inStock = $this->Stock->find('all', array('conditions' => array('Stock.material_id' => $key, 'Stock.publish' => 1, 'Stock.soft_delete' => 0, 'Stock.type' => 1, 'Stock.received_date' => date('Y-m-d', strtotime($startDate))),
                            'fields' => array('SUM(Stock.quantity) as qtyreceived'),
                            'recursive' => 0
                        ));
                        // debug($inStock);
                        $result['date'] = $startDate;
                        
                        if (isset($inStock) && $inStock[0][0]['qtyreceived'] != null){ 
                            $result['quantity'] = $result['quantity'] + $inStock[0][0]['qtyreceived'];
                        }else{
                            $result['quantity'] = $result['quantity'] + 0;
                        }
                            

                        if (isset($outStock)  && $outStock[0][0]['qtyconsumed'] != null ) {
                            $result['quantity_consumed'] = $outStock[0][0]['qtyconsumed'];
                            $result['quantity'] = $result['quantity'] + $inStock[0][0]['qtyconsumed'] - $result['quantity_consumed'];
                        } else {                            
                            $result['quantity_consumed'] = 0;
                            $result['quantity'] = $result['quantity'] + 0;
                        }
                        ...
                        $labels[] = date('d M Y',strtotime($result['date']));
                        $series1[] = $result['quantity_consumed'];
                        $series2[] = $result['quantity'];                        
                        $startDate = date("Y-m-d", strtotime("+1 day", strtotime($startDate)));
                    }
                $newResult[$value] = array('label'=> $labels,'series1'=>$series1,'series2'=>$series2);
                endforeach;
    ...
    public function get_material_details($id = null, $curStockId = null) {
        ...
        return array('material'=>$materialDetails['Material'], 'stock'=>$stock, 'product'=>$productDetails);
    }
    ...
    public function get_stock_details($id = null, $curStockId = null) {
        $this->autoRender = false;
        $this->loadModel('Material');
        $material = $this->Material->find('first',array('recursive'=>-1, 'conditions'=>array('Material.id'=>$id)));
        $stocks_in = $this->Stock->find('all', array(
            'conditions' => array('Stock.type'=>1, 'Stock.material_id' => $id),
            'recursive' => -1
        ));

        foreach ($stocks_in as $in) {
            $inStock = $in['Stock']['quantity'] + $inStock;
        }
        
        $stocks_out = $this->Stock->find('all', array(
            'conditions' => array('Stock.type'=>0, 'Stock.material_id' => $id),
            'recursive' => -1
        ));
        foreach ($stocks_out as $out) {
            $outStock = $out['Stock']['quantity_consumed'] + $outStock;
        }
        $stock = $inStock - $outStock;
        $this->set(compact('stock'));
        return array('stock'=>$stock,'material'=>$material['Material']);
        exit;
    }
