	...
	<?php echo $this->Form->create(array('class'=>'no-padding no-margin no-background'));?>
		<table cellpadding="0" cellspacing="0" class="table table-bordered table table-striped table-hover">
			<tr>
				...
				<th><?php echo $this->Paginator->sort('product_category_id', __('Product Category')); ?></th>
				<th><?php echo __('Required Material'); ?></th>
				<th><?php echo $this->Paginator->sort('branch_id', __('Branch')); ?></th>
				<th><?php echo $this->Paginator->sort('department_id'); ?></th>
				<th><?php echo $this->Paginator->sort('prepared_by'); ?></th>
				<th><?php echo $this->Paginator->sort('approved_by'); ?></th>
				<th><?php echo __('Add Weekly Plan'); ?></th>
				<th><?php echo $this->Paginator->sort('publish', __('Publish')); ?></th>
			</tr>
			<?php if($products){ ?>
<?php $x=0;
foreach ($products as $product): 
	if($product['Product']['nc_found'])$nc_class = 'text-danger';
	else $nc_class = 'text-default';
	?>
			<tr class="on_page_src <?php echo $nc_class; ?>">
                    <td class=" actions">
				    <?php echo  $this->element('actions',array('created'=>$product['Product']['created_by'],'postVal'=>$product['Product']['id'], 'softDelete'=>$product['Product']['soft_delete'],'nc_found'=>$product['Product']['nc_found'])); ?>
				</td>
				...
				<td><?php echo $this->Html->link('Add Weekly Plan',array('controller'=>'production_weekly_plans','action'=>'lists','product_id'=>$product['Product']['id']),array('class'=>'btn btn-xs btn-info'));?></td>
				<td width="60">
