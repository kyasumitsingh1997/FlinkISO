<?php
echo $this->Html->css(array('bootstrap-chosen.min','custom'));
echo $this->fetch('css');
?>
<div class="messages_reply">
    <div class="index reply">
        <div class="msg-box">
            ...
            <?php echo $this->Form->postLink(__('Delete'), array('action' => 'delete', $message['Message']['id']), array('class' => 'btn btn-sm btn-danger'), __('Are you sure you want to move "%s" to trash?', $message['Message']['subject'])); ?>
            <?php
                echo $this->HTML->link(__('Cancel'), array('controller' => 'messages', 'action' => 'index'), array('class' => 'btn btn-sm btn-default'));
            ?>
        </div>
        </div>
...
<script>
    $.validator.setDefaults({
        submitHandler: function(form) {
            $(form).ajaxSubmit({
                url: "<?php echo Router::url('/', true); ?><?php echo $this->request->params['controller'] ?>/reply/<?php echo $this->request->params['pass'][1];?>",
                type: 'POST',
