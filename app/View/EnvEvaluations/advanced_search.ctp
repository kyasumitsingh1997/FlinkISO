			...
			<div class="row">
				<div class="col-md-4 pull-right">
					<div class="input-group pull-left">
						<?php echo $this->Form->create('EnvEvaluation',array('role'=>'form','class'=>'no-padding no-margin','id'=>'search-form','action'=>'search')); ?>
						...
						<?php echo $this->Form->input(__('search'),array('label'=>false,'placeholder'=>'search','role'=>'form','class'=>'form-control','action'=>'search')); ?>
							...
							<div class="input-group-btn">
								...
								<ul class="dropdown-menu pull-right">
									...
									<li><?php echo $this->Js->submit('Submit',array('url'=>'search','div'=>false,'class'=>'btn btn-primary btn-success','update' => '#envEvaluations_ajax','type'=>'data','method'=>'post')); ?>
;									<?php echo $this->Form->end(); ?></li>
				...
				<div class="col-md-8">
					<h4><?php echo __('Env Evaluations'); ?>
...
<?php foreach ($envEvaluations as $envEvaluation): ?>
	<tr>
<td width="15"><?php echo $this->Form->checkbox('rec_ids_'.$i,array('label'=>false,'value'=>$envEvaluation['EnvEvaluation']['id'],'multiple'=>'checkbox','class'=>'rec_ids')); ?></td>		<td class=" actions">
		<div class="btn-group">
		<button type="button" data-toggle="dropdown" class="dropdown-toggle btn  btn-sm btn-default "><i class="fa fa-wrench" aria-hidden="true"></i></button>
				...
				<li><?php echo $this->Html->link(__('View / Upload Evidance'), array('action' => 'view', $envEvaluation['EnvEvaluation']['id'])); ?></li>
				<li><?php echo $this->Html->link(__('Edit'), array('action' => 'edit', $envEvaluation['EnvEvaluation']['id'])); ?></li>
				<li><?php echo $this->Form->postLink(__('Delete'), array('action' => 'delete', $envEvaluation['EnvEvaluation']['id']),array('class'=>''), __('Are you sure you want to delete this record ?', $envEvaluation['EnvEvaluation']['id'])); ?></li>
				<li class="divider"></li>
				<li><?php echo $this->Html->link(__('Send for Approval'), '#makerchecker',array('data-toggle'=>'modal')); ?></li>
				<li><?php echo $this->Form->postLink(__('Email Record'), array('controller'=>'message','action' => 'add', $envEvaluation['EnvEvaluation']['id']),array('class'=>''), __('Are you sure ?', $envEvaluation['EnvEvaluation']['id'])); ?></li>
			</ul>
		</div>
		</td>
		<td width="50"><?php echo h($envEvaluation['EnvEvaluation']['sr_no']); ?>&nbsp;</td>
		...
		<td>
			<?php echo $this->Html->link($envEvaluation['EnvIndentification']['title'], array('controller' => 'env_indentifications', 'action' => 'view', $envEvaluation['EnvIndentification']['id'])); ?>
		</td>
		...
		<td width="60">
			...
			<?php } ?>&nbsp;</td>
		<td><?php echo h($envEvaluation['EnvEvaluation']['record_status']); ?>&nbsp;</td>
		<td><?php echo h($envEvaluation['EnvEvaluation']['status_user_id']); ?>&nbsp;</td>
		<td>
			<?php echo $this->Html->link($envEvaluation['BranchIds']['name'], array('controller' => 'branches', 'action' => 'view', $envEvaluation['BranchIds']['id'])); ?>
		</td>
		<td>
			<?php echo $this->Html->link($envEvaluation['DepartmentIds']['name'], array('controller' => 'departments', 'action' => 'view', $envEvaluation['DepartmentIds']['id'])); ?>
		</td>
		<td>
			<?php echo $this->Html->link($envEvaluation['ApprovedBy']['name'], array('controller' => 'employees', 'action' => 'view', $envEvaluation['ApprovedBy']['id'])); ?>
		</td>
		<td>
			<?php echo $this->Html->link($envEvaluation['PreparedBy']['name'], array('controller' => 'employees', 'action' => 'view', $envEvaluation['PreparedBy']['id'])); ?>
		</td>
	</tr>
<?php endforeach; ?>
<?php }else{ ?>
...
<?php echo $this->Js->event('click',$this->Js->request(array('action' => 'add', 'ajax'),array('async' => true, 'update' => '#envEvaluations_ajax')));?>
...
<?php echo $this->Form->create('envEvaluations',array('action'=>'report','target'=>'_blank','class'=>'no-padding no-margin no-background zero-height'));?>
