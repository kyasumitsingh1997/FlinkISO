    ...
    */
    ...
    public function logout()
    {
        if ($this->Session->read('User.id')) {
            ...
            // $this->UserSession->read(null,$data['UserSession']['id']);
            // debug($this->UserSession);
            // $this->UserSession->save($data, false);
    ...
    public function dashboard()
    {
        ...
        //check if any updaes on help
        // $this->loadModel('Help');
        // $new_help = $this->Help->find('count', array(
        //     'conditions' => array(
        //         'OR' => array(
        //             'Help.created > ' => date('Y-m-d', strtotime($this->Session->read('User.lastLogin'))),
        //             'Help.modified > ' => date('Y-m-d', strtotime($this->Session->read('User.lastLogin')))
        //         )
        //     )
        // ));
        // if ($new_help > 0) {
        //     $this->set('help_update', true);
        //     $new_helps = $this->Help->find('all', array(
        //         'fields' => array(
        //             'Help.id',
        //             'Help.title',
        //             'Help.created',
        ...
        // if ($this->Session->read('User.is_mr') == 0 && $branchIDYes == true)
        //     $onlyBranch = array(
        //         'Task.branch_id' => $this->Session->read('User.branch_id')
        //     );
        // if ($this->Session->read('User.is_view_all') == 0)
        //     $onlyOwn = array(
        //         'Task.created_by' => $this->Session->read('User.id')
        //     );
        // if ($this->Session->read('User.is_mr') == 0) {
        //     $condition3 = array(
        //         'Task.user_id' => $this->Session->read('User.id')
        //     );
        // }
        ...
        //     else
        //         $condition1 = array(
        //             'Task.publish' => $this->request->params['named']['published']
        //         );
        //     if ($this->request->params['named']['soft_delete'] == null)
        //         $condition2 = null;
        //     else
        //         $condition2 = array(
        //             'Task.soft_delete' => $this->request->params['named']['soft_delete']
        //         );
        //     if ($this->request->params['named']['soft_delete'] == null)
        //         $conditions = array(
        //             $onlyBranch,
        ...
        //     )
        // );
        // $tasks_completed = $this->Task->find('count', $options);
        // $this->set('tasks_completed', $tasks_completed);
        ...
        // if ($this->Session->read('User.is_mr') == 0 && $branchIDYes == true)$onlyBranch = array('Task.branch_id' => $this->Session->read('User.branch_id'));
        // if ($this->Session->read('User.is_view_all') == 0)$onlyOwn = array('Task.created_by' => $this->Session->read('User.id'));
        // if ($this->Session->read('User.is_mr') == 0) {$condition3 = array('Task.user_id' => $this->Session->read('User.id'));}
        ...
        // $project_tasks = $this->Task->find('count', $options);
        // $this->set('project_tasks', $project_tasks);
        ...
        //     )
        // );
        // $project_tasks_completed = $this->Task->find('count', $options);
        // $this->set('project_tasks_completed', $project_tasks_completed);
        ...
        // $process_tasks = $this->Task->find('count', $options);
        // $this->set('process_tasks', $process_tasks);
        ...
        // $process_tasks_completed = $this->Task->find('count', $options);
        // $this->set('process_tasks_completed', $process_tasks_completed);
    ...
    public function _add_timeline()
    {
        ...
        /* FlinkISO End Date Timeline Entry */
        /*
        $this->Timeline->create();
        $timeline = array();
        $timeline['title'] = 'FlinkISO End Date';
        $timeline['message'] = "Your free subscribtion ends on " . date('d M Y', strtotime('+15 days')) . " !";
        $timeline['start_date'] = date('Y-m-d', strtotime('+1 month'));
        $timeline['end_date'] = date('Y-m-d', strtotime('+1 month'));
        $timeline['publish'] = 1;
        $timeline['soft_delete'] = 0;
        $timeline['company_id'] = $this->Session->read('User.company_id');
        $timeline['prepared_by'] = $this->Session->read('User.id');
...
class EmailConfig {
    ...
    public function a()
    {
        ...
        foreach ($models as $key => $model) {
            ...
            echo "<table border='1' width='100%' cellpadding='5'><tr><th colspan='2'><h3>" . $model . ": Belongs To</h3></th></tr><tr><th>Model</th><th>FK</th></tr>";
            foreach ($this->$model->belongsTo as $fkey => $fdetails) {
                echo "<tr><td width='50%'><h4>" . $fdetails['className'] . "</h4></td><td>" . $fdetails['foreignKey'] . "</td></tr>";
            }
            echo "<tr><th colspan='2'><h3>" . $model . ": Has Many</h3></th></tr><tr><th>Model</th><th>FK</th></tr>";
            foreach ($this->$model->hasMany as $hkey => $hdetails) {
                echo "<tr><td><h4>" . $hdetails['className'] . "</h4></td><td>" . $hdetails['foreignKey'] . "</td></tr>";
            }
            echo "</table><br />";
...
*/
        ...
        // debug($projectFile);
        // exit;
            // if not found .. search for queued fils
        ...
        if(!$projectFile){
            ...
            // if($projectFile && !isset($this->request->params['named']['project_file_id'])){
            //     $holdFile = $this->_get_files_on_hold($this->Session->read('User.employee_id'),$projectFile['ProjectFile']['milestone_id']);
            //     // print_r($holdFile);
            //     $this->set('holdFile',$holdFile);     
            // }else{
            //     $projectFile = $this->_get_files_on_hold($this->Session->read('User.employee_id'),$projectFile['ProjectFile']['milestone_id']);
            // }
        ...
        // $projectFileQs = $this->ProjectFile->find('all',array(
        //     'recursive'=>1,
        //     'conditions'=>array(
        //         'ProjectFile.pro_pub' =>1,
        //         'ProjectFile.pro_sd' =>0,
        ...
        //     )
        // ));
        // $this->set('projectFileQs',$projectFileQs);
        ...
        // debug(array_keys($projectProcesses));
        // debug($projectFile['ProjectFile']['milestone_id']);
        // exit;
        // $this->ProjectFile->FileProcess->virtualFields = array(
        //     'op1'=>'select count(*) from `project_overall_plans` where `project_overall_plans`.`id` LIKE  FileProcess.project_overall_plan_id'
        // );
        ...
        // // debug($currentProcesses['FileProcess']['milestone_id']);
        // // debug($projectFile['ProjectFile']['milestone_id']);
        // exit;
        if(!$currentProcesses){
            // create process
        ...
        $allprojects = $this->Project->find('list',array(
            'order'=>array('Project.start_date' => 'ASC'),
            'conditions'=>array(
                // 'or'=>array(
                //      'Project.userexists > '=> 0,
        ...
        foreach ($allprojects as $key => $value) {
            $requests[$key] = $this->ProjectReleaseRequest->find('all',array(
                'recursive'=>0,
                'fields'=>array(
        ...
        //     // echo $teamLeader['Employee']['id'].'<br />';
        //     $requests[$teamLeader['NewProject']['id']] = $this->ProjectReleaseRequest->find('all',array(
        //         'conditions'=>array(
        //             'ProjectReleaseRequest.employee_id'=>$teamLeader['Employee']['id'],
        //             'ProjectReleaseRequest.request_status'=>0,
        //             'ProjectReleaseRequest.new_project_id'=>$teamLeader['NewProject']['id']
        //         )
        //     ));
        // }
        ...
        $projectChecklists = $this->Project->ProjectChecklist->find('list',array(
            'conditions'=>array(
                'ProjectChecklist.project_id'=>$projectFile['ProjectFile']['project_id'],
                'ProjectChecklist.milestone_id'=>$projectFile['ProjectFile']['milestone_id'],
                'ProjectChecklist.soft_delete'=>0,
                'ProjectChecklist.project_process_plan_id' => $projectFile['ProjectFile']['curr_stage']
            )
        ));
        ...
        //     $act = $this->ProjectTimesheet->Project->ProjectActivity->find('all',array(
        //         'fields'=>array(
        //             'ProjectActivity.id',
        //             'ProjectActivity.title',
        //             'Milestone.id',
        ...
        //         ),
        //         'recursive'=>0,
        //         // 'group'=>array('ProjectActivity.milestone_id'),
