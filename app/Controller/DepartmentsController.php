...
class DepartmentsController extends AppController {
    ...
    public function edit($id = null) {
        ...
        if ($this->request->is('post') || $this->request->is('put')) {
            ...
            $this->request->data['Department']['system_table_id'] = $this->_get_system_table_id();
            if ($this->Department->save($this->request->data)) {
                if($this->request->data['Department']['old_name'] != $this->request->data['Department']['name']) $this->_update_folders($this->request->data['Department']);    
                $this->Session->setFlash(__('The department has been saved'));
    ...
    public function _update_folders($data = null){
        $this->loadModel('FileUpload');
        $files = $this->FileUpload->find('all',array(
            'recursive' => -1,            
            'conditions'=>array(
                'FileUpload.system_table_id'=>'dashboards', 
                'FileUpload.record' => $data['old_name']
            )));
        
        
        $department_folder = str_replace(' ', '_', $data['name']);
        $department_folder = strtolower($department_folder);
        
        foreach ($files as $file) {
            $dept_data['Department']['id'] = $data['id'];
            $file['FileUpload']['record'] = $data['name'];
            $file['FileUpload']['file_dir'] = str_replace($data['old_name'],$department_folder,$file['FileUpload']['file_dir']);
            $file['FileUpload']['comment'] = $file['FileUpload']['comment'] . '- Directory name changed';
            
            $this->FileUpload->read(null,$file['FileUpload']['id']);
            $this->FileUpload->save($file,false);
        }
        
        $dir_from = APP . 'files' . DS . $this->Session->read('User.company_id') . DS . 'upload' .  DS . 'documents' . DS .  $data['old_name'];
        $dir_to = APP . 'files' . DS . $this->Session->read('User.company_id') . DS . 'upload' .  DS . 'documents' . DS .  $department_folder;
        rename($dir_from, $dir_to);

        $dir_from = APP . 'files' . DS . $this->Session->read('User.company_id') . DS . 'revisions' .  DS . 'documents' . DS.  $data['old_name'];
        $dir_to = APP . 'files' . DS . $this->Session->read('User.company_id') . DS . 'revisions' .  DS . 'documents' . DS .  $department_folder;
        rename($dir_from, $dir_to);
