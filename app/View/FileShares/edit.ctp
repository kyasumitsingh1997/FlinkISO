...
<h4><?php echo __('Edit File Share'); ?>		
		<?php echo $this->Html->link(__('List'), array('action' => 'index'),array('id'=>'list','class'=>'label btn-info')); ?>
		
		</h4>
<?php echo $this->Form->create('FileShare',array('role'=>'form','class'=>'form')); ?>
<div class="row">
			<?php
		...
		echo "<div class='col-md-6'>".$this->Form->input('everyone') . '</div>'; 
...
<script>
    ...
    $().ready(function() {
        $('#FileShareEditForm').validate();
        $("#submit-indicator").hide();
        $("#submit_id").click(function(){
            if($('#FileShareEditForm').valid()){
                 $("#submit_id").prop("disabled",true);
                 $("#submit-indicator").show();
                $('#FileShareEditForm').submit();
            }
