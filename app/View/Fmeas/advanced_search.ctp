...
<?php foreach ($fmeas as $fmea): ?>
	<tr>
<td width="15"><?php echo $this->Form->checkbox('rec_ids_'.$i,array('label'=>false,'value'=>$fmea['Fmea']['id'],'multiple'=>'checkbox','class'=>'rec_ids')); ?></td>		<td class=" actions">
		<div class="btn-group">
		<button type="button" data-toggle="dropdown" class="dropdown-toggle btn  btn-sm btn-default "><span class=" glyphicon glyphicon-wrench"></span></button>
				...
				<li><?php echo $this->Html->link(__('View / Upload Evidance'), array('action' => 'view', $fmea['Fmea']['id'])); ?></li>
...
<?php echo $this->Form->create('fmeas',array('action'=>'report','target'=>'_blank','class'=>'no-padding no-margin no-background zero-height'));?>
